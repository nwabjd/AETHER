...
`;let w=null,ke=null;function B(e,t=""){if(!ke)return;const r=document.createElement("div");r.className=`log-entry ${t}`,r.textContent=e,ke.appendChild(r),ke.scrollTop=ke.scrollHeight}async function Qt(){B("═══ TINY NEURAL NETWORK TEST ═══","info"),B("Architecture: input(4) → linear(4,3) → ReLU → linear(3,1)","info"),B("");const e=await Pe();if(!e)return B("WebGPU not available","err"),!1;w=await $e(e),Ue(w);const t=performance.now(),r=T.fromData(w,new Float32Array([1,.5,-.3,.8]),[4]),n=T.fromData(w,new Float32Array([.2,-.4,.1,.5,.3,-.2,-.1,.6,.4,.3,-.1,.5]),[4,3]),a=T.fromData(w,new Float32Array([.1,-.1,.2]),[3]),s=new ArrayBuffer(12),i=new Uint32Array(s);i[0]=1,i[1]=3,i[2]=4;const o=w.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:3,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}}]}),u=w.createComputePipeline({layout:w.createPipelineLayout({bindGroupLayouts:[o]}),compute:{module:w.createShaderModule({code:mt}),entryPoint:"main"}}),l=w.createBuffer({size:12,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});w.queue.writeBuffer(l,0,s);const c=new T(w,[1,3]),d=w.createBindGroup({layout:o,entries:[{binding:0,resource:{buffer:l}},{binding:1,resource:{buffer:r.buffer}},{binding:2,resource:{buffer:n.buffer}},{binding:3,resource:{buffer:c.buffer}}]});let p=w.createCommandEncoder(),f=p.beginComputePass();f.setPipeline(u),f.setBindGroup(0,d),f.dispatchWorkgroups(1,1,1),f.end(),w.queue.submit([p.finish()]),B(`  input[4]:  [${Array.from(await r.readback()).map(G=>G.toFixed(2)).join(", ")}]`,""),B("  W1[4×3]:   4 rows × 3 cols",""),B("  Matmul result: computing...","");const m=await c.readback();B(`  h1 = input @ W1: [${Array.from(m).map(G=>G.toFixed(3)).join(", ")}]`,"ok");for(let G=0;G<3;G++)m[G]+=[.1,-.1,.2][G];w.queue.writeBuffer(c.buffer,0,m.buffer),B(`  h1 + bias:       [${Array.from(m).map(G=>G.toFixed(3)).join(", ")}]`,"ok");const g=w.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}}]}),b=w.createComputePipeline({layout:w.createPipelineLayout({bindGroupLayouts:[g]}),compute:{module:w.createShaderModule({code:yn}),entryPoint:"main"}}),v=new ArrayBuffer(4);new Uint32Array(v)[0]=3;const h=w.createBuffer({size:4,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});w.queue.writeBuffer(h,0,v);const y=w.createBindGroup({layout:g,entries:[{binding:0,resource:{buffer:h}},{binding:1,resource:{buffer:c.buffer}}]});p=w.createCommandEncoder(),f=p.beginComputePass(),f.setPipeline(b),f.setBindGroup(0,y),f.dispatchWorkgroups(1,1,1),f.end(),w.queue.submit([p.finish()]);const E=await c.readback();B(`  ReLU(h1):         [${Array.from(E).map(G=>G.toFixed(3)).join(", ")}]`,"ok");const x=T.fromData(w,new Float32Array([.7,-.3,.5]),[3,1]),k=new T(w,[1,1]),O=new ArrayBuffer(12),F=new Uint32Array(O);F[0]=1,F[1]=1,F[2]=3;const R=w.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:3,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}}]}),X=w.createComputePipeline({layout:w.createPipelineLayout({bindGroupLayouts:[R]}),compute:{module:w.createShaderModule({code:mt}),entryPoint:"main"}}),Ne=w.createBuffer({size:12,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});w.queue.writeBuffer(Ne,0,O);const me=w.createBindGroup({layout:R,entries:[{binding:0,resource:{buffer:Ne}},{binding:1,resource:{buffer:c.buffer}},{binding:2,resource:{buffer:x.buffer}},{binding:3,resource:{buffer:k.buffer}}]});p=w.createCommandEncoder(),f=p.beginComputePass(),f.setPipeline(X),f.setBindGroup(0,me),f.dispatchWorkgroups(1,1,1),f.end(),w.queue.submit([p.finish()]);const Z=await k.readback(),ie=(performance.now()-t).toFixed(1);return B(`  Final output: ${Z[0].toFixed(4)}`,"ok"),B(`  Total pipeline: ${ie} ms`,"ok"),B("",""),B("✓ Tiny NN passed: JavaScript → WebGPU → compute shader → tensor → result","ok"),r.destroy(),n.destroy(),a.destroy(),c.destroy(),x.destroy(),k.destroy(),l.destroy(),Ne.destroy(),h.destroy(),w.destroy(),!0}async function Mn(){B("═══ MATRIX MULTIPLICATION BENCHMARK ═══","info");const e=await Pe();if(!e)return null;w=await $e(e),Ue(w);const t=[64,128,256,512],r=[];for(const n of t){const a=T.fromData(w,new Float32Array(n*n).fill(1),[n,n]),s=T.fromData(w,new Float32Array(n*n).fill(.5),[n,n]),i=new T(w,[n,n]),o=w.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:3,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}}]}),u=w.createComputePipeline({layout:w.createPipelineLayout({bindGroupLayouts:[o]}),compute:{module:w.createShaderModule({code:mt}),entryPoint:"main"}}),l=new ArrayBuffer(12),c=new Uint32Array(l);c[0]=n,c[1]=n,c[2]=n;const d=await je(`${n}×${n} matmul`,async()=>{const p=w.createBuffer({size:12,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});w.queue.writeBuffer(p,0,l);const f=w.createBindGroup({layout:o,entries:[{binding:0,resource:{buffer:p}},{binding:1,resource:{buffer:a.buffer}},{binding:2,resource:{buffer:s.buffer}},{binding:3,resource:{buffer:i.buffer}}]}),m=w.createCommandEncoder(),g=m.beginComputePass();g.setPipeline(u),g.setBindGroup(0,f);const b=Math.ceil(n/16);g.dispatchWorkgroups(b,b,1),g.end(),w.queue.submit([m.finish()]),p.destroy()},30,2*n*n*n);r.push(d),B(Ve(d),"ok"),a.destroy(),s.destroy(),i.destroy()}return w.destroy(),r[r.length-1]}async function En(){B("═══ CONVOLUTION BENCHMARK ═══","info");const e=await Pe();if(!e)return null;w=await $e(e),Ue(w);const t=1,r=3,n=32,a=32,s=8,i=3,o=3,u=n-i+1,l=a-o+1,c=T.fromData(w,new Float32Array(t*r*n*a).fill(.5),[t,r,n,a]),d=T.fromData(w,new Float32Array(s*r*i*o).fill(.1),[s,r,i,o]),p=new T(w,[t,s,u,l]),f=w.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:3,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}}]}),m=w.createComputePipeline({layout:w.createPipelineLayout({bindGroupLayouts:[f]}),compute:{module:w.createShaderModule({code:hn}),entryPoint:"main"}}),g=new ArrayBuffer(36),b=new Uint32Array(g);b[0]=t,b[1]=r,b[2]=n,b[3]=a,b[4]=s,b[5]=i,b[6]=o,b[7]=u,b[8]=l;const v=await je(`Conv2D ${t}×${r}×${n}×${a} k=${i}→${s}×${u}×${l}`,async()=>{const h=w.createBuffer({size:36,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});w.queue.writeBuffer(h,0,g);const y=w.createBindGroup({layout:f,entries:[{binding:0,resource:{buffer:h}},{binding:1,resource:{buffer:c.buffer}},{binding:2,resource:{buffer:d.buffer}},{binding:3,resource:{buffer:p.buffer}}]}),E=w.createCommandEncoder(),x=E.beginComputePass();x.setPipeline(m),x.setBindGroup(0,y),x.dispatchWorkgroups(t,s,1),x.end(),w.queue.submit([E.finish()]),h.destroy()},20,2*t*s*r*i*o*u*l);return B(Ve(v),"ok"),c.destroy(),d.destroy(),p.destroy(),w.destroy(),v}async function An(){B("═══ ATTENTION BENCHMARK ═══","info");const e=await Pe();if(!e)return null;w=await $e(e),Ue(w);const t=1,r=64,n=64,a=1/Math.sqrt(n),s=T.fromData(w,new Float32Array(t*r*n).fill(.1),[t,r,n]),i=T.fromData(w,new Float32Array(t*r*n).fill(.1),[t,r,n]),o=T.fromData(w,new Float32Array(t*r*n).fill(.1),[t,r,n]),u=new T(w,[t,r,n]),l=new T(w,[t,r,r]),c=w.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:3,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:4,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}},{binding:5,visibility:GPUShaderStage.COMPUTE,buffer:{type:"storage"}}]}),d=w.createComputePipeline({layout:w.createPipelineLayout({bindGroupLayouts:[c]}),compute:{module:w.createShaderModule({code:wn}),entryPoint:"main"}}),p=new ArrayBuffer(16),f=new Uint32Array(p),m=new Float32Array(p);f[0]=t,f[1]=r,f[2]=n,m[3]=a;const g=await je(`Attention b=${t} s=${r} d=${n}`,async()=>{const b=w.createBuffer({size:16,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});w.queue.writeBuffer(b,0,p);const v=w.createBindGroup({layout:c,entries:[{binding:0,resource:{buffer:b}},{binding:1,resource:{buffer:s.buffer}},{binding:2,resource:{buffer:i.buffer}},{binding:3,resource:{buffer:o.buffer}},{binding:4,resource:{buffer:u.buffer}},{binding:5,resource:{buffer:l.buffer}}]}),h=w.createCommandEncoder(),y=h.beginComputePass();y.setPipeline(d),y.setBindGroup(0,v),y.dispatchWorkgroups(t,1,1),y.end(),w.queue.submit([h.finish()]),b.destroy()},20);return B(Ve(g),"ok"),s.destroy(),i.destroy(),o.destroy(),u.destroy(),l.destroy(),w.destroy(),g}function Pn(e){e.innerHTML=`
    <h2>Model Test</h2>
...
`,"");const t=[];{const i=A.randn([64,64]),o=A.randn([64,64]),u=await i.readback(),l=await o.readback(),c=await V("matmul 64",()=>_e(u,l,64,64,64)),d=await K("matmul 64",async()=>{(await Re(i,o,64,64,64)).destroy()}),p=await(await Re(i,o,64,64,64)).readback(),f=_e(u,l,64,64,64),m=j(f,p),g=Math.max(...Array.from(f).map((b,v)=>Math.abs(b-p[v])));t.push({name:"Matmul",shape:"64×64 @ 64×64",cpuMs:c,gpuMs:d,speedup:c/d,correct:m,tolerance:g}),i.destroy(),o.destroy()}{const i=A.randn([256,256]),o=A.randn([256,256]),u=await i.readback(),l=await o.readback(),c=await V("matmul 256",()=>_e(u,l,256,256,256),10),d=await K("matmul 256",async()=>{(await Re(i,o,256,256,256)).destroy()}),p=await(await Re(i,o,256,256,256)).readback(),f=_e(u,l,256,256,256),m=j(f,p),g=Math.max(...Array.from(f).map((b,v)=>Math.abs(b-p[v])));t.push({name:"Matmul",shape:"256×256 @ 256×256",cpuMs:c,gpuMs:d,speedup:c/d,correct:m,tolerance:g}),i.destroy(),o.destroy()}{const i=A.randn([512,512]),o=A.randn([512,512]),u=await i.readback(),l=await o.readback(),c=await V("matmul 512",()=>_e(u,l,512,512,512),5),d=await K("matmul 512",async()=>{(await Re(i,o,512,512,512)).destroy()}),p=await(await Re(i,o,512,512,512)).readback(),f=_e(u,l,512,512,512),m=j(f,p),g=Math.max(...Array.from(f).map((b,v)=>Math.abs(b-p[v])));t.push({name:"Matmul",shape:"512×512 @ 512×512",cpuMs:c,gpuMs:d,speedup:c/d,correct:m,tolerance:g}),i.destroy(),o.destroy()}{const a=A.randn([1e6]),s=A.randn([1e6]),i=await a.readback(),o=await s.readback(),u=await V("add 1M",()=>tr(i,o)),l=await K("add 1M",async()=>{(await er(a,s)).destroy()}),c=await(await er(a,s)).readback(),d=tr(i,o),p=j(d,c),f=Math.max(...Array.from(d).map((m,g)=>Math.abs(m-c[g])));t.push({name:"Add",shape:"[1000000]",cpuMs:u,gpuMs:l,speedup:u/l,correct:p,tolerance:f}),a.destroy(),s.destroy()}{const a=A.randn([1e6]),s=A.randn([1e6]),i=await a.readback(),o=await s.readback(),u=await V("mul 1M",()=>nr(i,o)),l=await K("mul 1M",async()=>{(await rr(a,s)).destroy()}),c=await(await rr(a,s)).readback(),d=nr(i,o),p=j(d,c),f=Math.max(...Array.from(d).map((m,g)=>Math.abs(m-c[g])));t.push({name:"Multiply",shape:"[1000000]",cpuMs:u,gpuMs:l,speedup:u/l,correct:p,tolerance:f}),a.destroy(),s.destroy()}{const a=A.randn([1024]),s=A.ones([1024]),i=await a.readback(),o=await s.readback(),u=await V("rmsnorm",()=>sr(i,o)),l=await K("rmsnorm",async()=>{(await ar(a,s)).destroy()}),c=await(await ar(a,s)).readback(),d=sr(i,o),p=j(d,c),f=Math.max(...Array.from(d).map((m,g)=>Math.abs(m-c[g])));t.push({name:"RMSNorm",shape:"[1024]",cpuMs:u,gpuMs:l,speedup:u/l,correct:p,tolerance:f}),a.destroy(),s.destroy()}{const a=A.randn([1024]),s=A.ones([1024]),i=A.zeros([1024]),o=await a.readback(),u=await s.readback(),l=await i.readback(),c=await V("layernorm",()=>ir(o,u,l)),d=await K("layernorm",async()=>{(await or(a,s,i)).destroy()}),p=await(await or(a,s,i)).readback(),f=ir(o,u,l),m=j(f,p),g=Math.max(...Array.from(f).map((b,v)=>Math.abs(b-p[v])));t.push({name:"LayerNorm",shape:"[1024]",cpuMs:c,gpuMs:d,speedup:c/d,correct:m,tolerance:g}),a.destroy(),s.destroy(),i.destroy()}{const s=A.randn([32,128]),i=await s.readback(),o=await V("softmax",()=>cr(new Float32Array(i),32,128)),u=await K("softmax",async()=>{(await ur(A.fromFloat32(new Float32Array(i),[32,128]),32,128)).destroy()}),l=await(await ur(A.fromFloat32(new Float32Array(i),[32,128]),32,128)).readback(),c=cr(new Float32Array(i),32,128),d=j(c,l),p=Math.max(...Array.from(c).map((f,m)=>Math.abs(f-l[m])));t.push({name:"Softmax",shape:"[32, 128]",cpuMs:o,gpuMs:u,speedup:o/u,correct:d,tolerance:p}),s.destroy()}{const s=A.randn([16,128]),i=await s.readback(),o=await V("rope",()=>dr(new Float32Array(i),16,128)),u=await K("rope",async()=>{(await lr(A.fromFloat32(new Float32Array(i),[16,128]),16,128)).destroy()}),l=await(await lr(A.fromFloat32(new Float32Array(i),[16,128]),16,128)).readback(),c=dr(new Float32Array(i),16,128),d=j(c,l),p=Math.max(...Array.from(c).map((f,m)=>Math.abs(f-l[m])));t.push({name:"RoPE",shape:"[16, 128]",cpuMs:o,gpuMs:u,speedup:o/u,correct:d,tolerance:p}),s.destroy()}{const c=A.randn([1,3,16,16]),d=A.randn([4,3,3,3]),p=await c.readback(),f=await d.readback(),m=await V("conv2d",()=>pr(p,f,1,3,16,16,4,3,3)),g=await K("conv2d",async()=>{(await fr(c,d,1,3,16,16,4,3,3)).destroy()}),b=await(await fr(c,d,1,3,16,16,4,3,3)).readback(),v=pr(p,f,1,3,16,16,4,3,3),h=j(v,b),y=Math.max(...Array.from(v).map((E,x)=>Math.abs(E-b[x])));t.push({name:"Conv2D",shape:"[1,3,16,16] k=3→4",cpuMs:m,gpuMs:g,speedup:m/g,correct:h,tolerance:y}),c.destroy(),d.destroy()}{const s=A.randn([256,256]),i=await s.readback(),o=await V("transpose",()=>gr(i,256,256)),u=await K("transpose",async()=>{(await mr(s,256,256)).destroy()}),l=await(await mr(s,256,256)).readback(),c=gr(i,256,256),d=j(c,l),p=Math.max(...Array.from(c).map((f,m)=>Math.abs(f-l[m])));t.push({name:"Transpose",shape:"[256, 256]",cpuMs:o,gpuMs:u,speedup:o/u,correct:d,tolerance:p}),s.destroy()}{const u=A.randn([3072]),l=await u.readback(),c=await V("interp",()=>vr(l,32,32,64,64,3)),d=await K("interp",async()=>{(await br(u,32,32,64,64,3)).destroy()}),p=await(await br(u,32,32,64,64,3)).readback(),f=vr(l,32,32,64,64,3),m=j(f,p),g=Math.max(...Array.from(f).map((b,v)=>Math.abs(b-p[v])));t.push({name:"Interpolate",shape:"32×32 → 64×64 ch=3",cpuMs:c,gpuMs:d,speedup:c/d,correct:m,tolerance:g}),u.destroy()}J("",""),J("═══ RESULTS ═══","info");for(const n of t){ua(n);const a=n.correct?"✓":"✗",s=n.correct?"ok":"err";J(`${a} ${n.name} (${n.shape}): CPU ${n.cpuMs.toFixed(2)} ms | GPU ${n.gpuMs.toFixed(2)} ms | ${n.speedup.toFixed(1)}× | max diff ${n.tolerance.toExponential(1)}`,s)}const r=t.filter(n=>n.correct).length;J("",""),J(`═══ ${r}/${t.length} CORRECT ═══`,r===t.length?"ok":"err"),Fn()}function la(e){e.innerHTML=`
