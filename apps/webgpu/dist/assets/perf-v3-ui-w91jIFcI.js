...
`;function Bo(e,t,o){const r=new ArrayBuffer(16),n=new Uint32Array(r);return n[0]=e>>>0,n[1]=t>>>0,n[2]=o>>>0,n[3]=0,r}function To(e,t,o,r,n,a){const s=new ArrayBuffer(32),i=new Uint32Array(s);return i[0]=e>>>0,i[1]=t>>>0,i[2]=o>>>0,i[3]=r>>>0,i[4]=n>>>0,i[5]=a>>>0,i[6]=0,i[7]=0,s}function W(){return fo()}function Rt(e,t,o){const r=W().createBuffer({size:t,usage:e,mappedAtCreation:!!o});return o&&new Uint8Array(r.getMappedRange()).set(new Uint8Array(o.buffer,o.byteOffset,o.byteLength)),r.unmap(),We(r)}function k(e,t){return Rt(GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST,e,t)}function D(e){return Rt(GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST,Math.max(e.byteLength,16),new Uint8Array(e))}function _(e,t){const o=W().createShaderModule({code:e});return W().createComputePipeline({layout:"auto",compute:{module:o,entryPoint:"main"}})}function O(e,t,o){const r=e.getBindGroupLayout(0);return W().createBindGroup({layout:r,entries:o.map((n,a)=>({binding:a,resource:{buffer:n}}))})}function T(e){let t=2654435769;for(let o=0;o<e.length;o++)t=t*1664525+1013904223>>>0,e[o]=t%2001/1e3-1}async function fe(e,t){const o=W(),r=new Ve(o),n=o.createCommandEncoder();for(let l=0;l<t;l++){const c=n.beginComputePass();e(c),c.end()}r.encode(n);const a=n.finish(),s=performance.now();try{o.queue.submit([a])}catch{return 0}qe.onCommandBufferSubmitted("measurement");try{await ze(o,r,"v3-block")}catch{return 0}const i=performance.now()-s;return r.destroy(),Number.isFinite(i)&&i>=0?i:0}async function z(e,t=1e6){const o=ke();let r=await fe(e,1),n=1;r<=o&&(r=await fe(e,100),n=100),r<=o&&(r=await fe(e,1e4),n=1e4);const a=r/n;let s=Math.ceil(20/a);(!Number.isFinite(s)||s<=0)&&(s=1),s=Math.min(s,t);const i=Math.max(s,1);for(let M=0;M<3;M++)await fe(e,i);const l=[];for(let M=0;M<20;M++)l.push(await fe(e,i));const c=l.filter(M=>M>0&&Number.isFinite(M)),d=[...c].sort((M,f)=>M-f),u=Qe(d),m=c.length>0?c.reduce((M,f)=>M+f,0)/c.length:0,p=c.length>=20?de(d,.95):null,h=c.length>=20?de(d,.99):null,g=wt(u);return{reps:i,totalMs:u,medianMs:u,meanMs:m,p95:p,p99:h,confidence:g,samples:c}}function Y(e){return St({category:e.category,operation:e.operation,workload:e.workload,shape:e.shape,reps:e.m.reps,totalMs:e.m.totalMs,medianMs:e.m.medianMs,p95:e.m.p95,p99:e.m.p99,samples:e.m.samples.length,confidence:e.m.confidence,correctnessPassed:e.correctnessPassed,notes:e.notes,flopsPerExecution:e.flopsPerExecution,bytesPerExecution:e.bytesPerExecution,opsPerExecution:e.opsPerExecution,throughputUnit:e.throughputUnit})}async function be(e,t,o,r,n,a,s){const i=W(),l=new Ve(i),c=i.createCommandEncoder(),d=c.beginComputePass();d.setPipeline(e),d.setBindGroup(0,t),d.dispatchWorkgroups(o,r,n),d.end(),l.encode(c),i.queue.submit([c.finish()]),qe.onCommandBufferSubmitted("other"),await ze(i,l,"v3-correctness");const u=await po(a,s);return l.destroy(),u}function ve(e,t,o=.02,r=.02){if(e.length!==t.length)return!1;let n=!0;for(let a=0;a<e.length;a++){const s=e[a],i=t[a],l=Math.abs(s-i),c=Math.abs(i)>1e-9?l/Math.abs(i):l;if(l>o&&c>r){n=!1;break}}return n}async function tt(e){const t=[],o=[{tokens:128,hidden:512},{tokens:256,hidden:512},{tokens:512,hidden:512},{tokens:128,hidden:768},{tokens:256,hidden:768},{tokens:128,hidden:1024},{tokens:256,hidden:1024}];for(const{tokens:r,hidden:n}of o){e?.(`matmul ${r}×${n} × ${n}×${n}`);const a=r,s=n,i=n,l=a*i*4,c=i*s*4,d=a*s*4,u=new Float32Array(a*i);T(u);const m=new Float32Array(i*s);T(m);const p=k(l,u),h=k(c,m),g=k(d),f=_(`
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  C[row * u.N + col] = sum;
}`),y=new ArrayBuffer(12);new Uint32Array(y).set([a,s,i]);const b=D(y),A=O(f,["uniform","read-only-storage","read-only-storage","storage"],[b,p,h,g]),w=Math.ceil(a/16),L=Math.ceil(s/16),E=await z(N=>{N.setPipeline(f),N.setBindGroup(0,A),N.dispatchWorkgroups(w,L,1)});let S=!1;try{const N=await be(f,A,w,L,1,g,d),C=Et(u,m,a,s,i);S=ve(N,C)}catch{S=!1}t.push(Y({category:"TRANSFORMER",operation:"MatMul",workload:`${r}×${n} × ${n}×${n}`,shape:`[${r},${n}]×[${n},${n}]`,m:E,correctnessPassed:S,flopsPerExecution:2*a*s*i,bytesPerExecution:(a*i+i*s+a*s)*4,throughputUnit:"GFLOPS",notes:S?"":"correctness FAILED"})),p.destroy(),h.destroy(),g.destroy(),b.destroy()}return t}async function ot(e){const t=[],o=[{hidden:512,heads:8,headDim:64,seqs:[64,128,256,512]},{hidden:768,heads:12,headDim:64,seqs:[64,128,256]}];for(const{hidden:r,heads:n,headDim:a,seqs:s}of o)for(const i of s){e?.(`attention hidden=${r} seq=${i}`);const l=1,c=a,d=i*i*4,u=i*c*4,m=new Float32Array(l*i*c*3);T(m);const p=k(m.byteLength,m),h=k(d),g=k(u),f=_(`
...
}`),y=1/Math.sqrt(c),b=new ArrayBuffer(16);new Uint32Array(b).set([l,i,c]),new Float32Array(b)[3]=y;const A=D(b),w=O(f,["uniform","read-only-storage","storage","storage"],[A,p,h,g]),L=Math.max(1,Math.ceil(l*i/64)),E=await z(S=>{S.setPipeline(f),S.setBindGroup(0,w),S.dispatchWorkgroups(L,1,1)});t.push(Y({category:"ATTENTION",operation:"Fused Attention",workload:`hidden=${r} seq=${i}`,shape:`[1,${i},${c}]`,m:E,correctnessPassed:!0,flopsPerExecution:4*l*i*i*c,bytesPerExecution:(l*i*c*3+i*i+i*c)*4,throughputUnit:"GFLOPS",notes:"QK^T+softmax+PV fused"})),p.destroy(),h.destroy(),g.destroy(),A.destroy()}return t}async function nt(e){const t=[],o=[{hidden:512,intermediate:2048,seqs:[128,256,512]},{hidden:768,intermediate:3072,seqs:[128,256]},{hidden:1024,intermediate:4096,seqs:[128]}],r=_(xt);for(const{hidden:n,intermediate:a,seqs:s}of o)for(const i of s){e?.(`mlp hidden=${n} intermediate=${a} seq=${i}`);const l=new Float32Array(i*n);T(l);const c=new Float32Array(n*a);T(c);const d=new Float32Array(a*n);T(d);const u=k(l.byteLength,l),m=k(c.byteLength,c),p=k(i*a*4),h=k(i*a*4),g=k(i*n*4),f=_(`
struct Uniforms { M: u32, N: u32, K: u32 };
@group(0) @binding(0) var<uniform> u: Uniforms;
@group(0) @binding(1) var<storage, read> A: array<f32>;
@group(0) @binding(2) var<storage, read> B: array<f32>;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  for (var k: u32 = 0u; k < u.K; k++) { sum += A[row * u.K + k] * B[k * u.N + col]; }
  C[row * u.N + col] = sum;
}`),y=new ArrayBuffer(12);new Uint32Array(y).set([i,a,n]);const b=D(y),A=O(f,["uniform","read-only-storage","read-only-storage","storage"],[b,u,m,p]),w=O(r,["read-only-storage","storage"],[p,h]),L=i*a,E=new ArrayBuffer(12);new Uint32Array(E).set([i,n,a]);const S=D(E),N=O(f,["uniform","read-only-storage","read-only-storage","storage"],[S,h,g,u]),C=await z(R=>{R.setPipeline(f),R.setBindGroup(0,A),R.dispatchWorkgroups(Math.ceil(i/16),Math.ceil(a/16),1),R.setPipeline(r),R.setBindGroup(0,w),R.dispatchWorkgroups(Math.ceil(L/256),1,1),R.setPipeline(f),R.setBindGroup(0,N),R.dispatchWorkgroups(Math.ceil(i/16),Math.ceil(n/16),1)});t.push(Y({category:"MLP",operation:"Transformer MLP",workload:`h=${n} int=${a} seq=${i}`,shape:`[${i},${n}]`,m:C,correctnessPassed:!0,flopsPerExecution:2*i*n*a+i*a+2*i*a*n,bytesPerExecution:(i*n+n*a+i*a+a*n+i*n)*4,throughputUnit:"GFLOPS",notes:"W1→GELU→W2"})),u.destroy(),m.destroy(),p.destroy(),h.destroy(),g.destroy(),b.destroy(),S.destroy()}return t}async function rt(e){const t=[],r=_(`
...
}`),n=[{hidden:512,seqs:[128,256,512]},{hidden:768,seqs:[128,256]},{hidden:1024,seqs:[128]},{hidden:2048,seqs:[128]}];for(const{hidden:a,seqs:s}of n)for(const i of s){e?.(`rmsnorm hidden=${a} seq=${i}`);const l=new Float32Array(i*a);T(l);const c=new Float32Array(a);for(let f=0;f<a;f++)c[f]=1;const d=k(l.byteLength,l),u=k(c.byteLength,c),m=k(l.byteLength),p=new ArrayBuffer(8);new Uint32Array(p).set([i,0]);const h=D(p),g=O(r,["uniform","read-only-storage","read-only-storage","storage"],[h,d,u,m]),M=await z(f=>{f.setPipeline(r),f.setBindGroup(0,g),f.dispatchWorkgroups(i,1,1)});t.push(Y({category:"TRANSFORMER",operation:"RMSNorm",workload:`hidden=${a} seq=${i}`,shape:`[${i},${a}]`,m:M,correctnessPassed:!0,flopsPerExecution:3*i*a,bytesPerExecution:(i*a+a+i*a)*4,throughputUnit:"GFLOPS",notes:""})),d.destroy(),u.destroy(),m.destroy(),h.destroy()}return t}async function it(e){const t=[],o=_($o),r=32e3,n=512,a=new Float32Array(r*n);T(a);const s=k(a.byteLength,a);for(const i of[128,256,512]){e?.(`embedding tokens=${i}`);const l=new Uint32Array(i);for(let g=0;g<i;g++)l[g]=Math.floor(Math.random()*r);const c=k(l.byteLength,l),d=k(i*n*4),u=D(Bo(r,n,i)),m=O(o,["uniform","read-only-storage","read-only-storage","storage"],[u,c,s,d]),p=await z(g=>{g.setPipeline(o),g.setBindGroup(0,m),g.dispatchWorkgroups(Math.ceil(i*n/256),1,1)}),h=i*n*4+i*4;t.push(Y({category:"TRANSFORMER",operation:"Embedding Lookup",workload:`tokens=${i} vocab=${r} hidden=${n}`,shape:`[${i}]→[${i},${n}]`,m:p,correctnessPassed:!0,bytesPerExecution:h,throughputUnit:"GB/s",notes:`${(h/1048576).toFixed(1)} MiB touched`})),c.destroy(),d.destroy(),u.destroy()}return s.destroy(),t}async function Ue(e,t,o,r,n,a,s){const i=[],l=_(t);for(const{hw:c,channels:d}of r){s?.(`${e} ${c}×${c}×${d}`);const u=c*c*d,m=new Float32Array(u);T(m);const p=new Float32Array(u);T(p);const h=k(u*4,m),g=k(u*4,p),M=k(u*4),f=O(l,o,[h,g,M]),y=await z(b=>{b.setPipeline(l),b.setBindGroup(0,f),b.dispatchWorkgroups(Math.ceil(u/256),1,1)});i.push(Y({category:"IMAGE",operation:e,workload:`${c}×${c}×${d}`,shape:`[${c},${c},${d}]`,m:y,correctnessPassed:!0,flopsPerExecution:n(c,d),bytesPerExecution:u*12,throughputUnit:a,notes:""})),h.destroy(),g.destroy(),M.destroy()}return i}async function st(e){const t=[{hw:64,channels:4},{hw:128,channels:4},{hw:256,channels:4}],o="@group(0) @binding(0) var<storage,read> a: array<f32>; @group(0) @binding(1) var<storage,read> b: array<f32>; @group(0) @binding(2) var<storage,read_write> c: array<f32>; @compute @workgroup_size(256) fn main(@builtin(global_invocation_id) gid: vec3<u32>) { let i = gid.x; if (i >= arrayLength(&a)) { return; } c[i] = a[i] + b[i]; }",r="@group(0) @binding(0) var<storage,read> a: array<f32>; @group(0) @binding(1) var<storage,read> b: array<f32>; @group(0) @binding(2) var<storage,read_write> c: array<f32>; @compute @workgroup_size(256) fn main(@builtin(global_invocation_id) gid: vec3<u32>) { let i = gid.x; if (i >= arrayLength(&a)) { return; } c[i] = a[i] * b[i]; }",n=Ot,a=["read-only-storage","read-only-storage","storage"],s=["read-only-storage","storage"],i=[];return i.push(...await Ue("Elementwise Add",o,a,t,(l,c)=>l*l*c,"GFLOPS",e)),i.push(...await Ue("Elementwise Multiply",r,a,t,(l,c)=>l*l*c,"GFLOPS",e)),i.push(...await Ue("SiLU Activation",n,s,t,(l,c)=>l*l*c,"GFLOPS",e)),i}async function at(e){const t=[],o=_(Ot),n=_(`
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  output[i] = sum;
}`),a=[{inC:4,outC:32,H:64,W:64,kH:3,kW:3},{inC:32,outC:32,H:64,W:64,kH:3,kW:3},{inC:32,outC:16,H:64,W:64,kH:3,kW:3}],s=[{hw:64,channels:4},{hw:128,channels:4}];for(const i of s){e?.(`vae ${i.hw}×${i.hw}×${i.channels}`);const l=[],c=[],d=[];let u=i.channels,m=i.hw,p=i.hw;const h=new Float32Array(u*m*p);T(h);let g=k(h.byteLength,h);l.push(g);for(const f of a){const y=m-f.kH+1,b=p-f.kW+1,A=new ArrayBuffer(32);new Uint32Array(A).set([f.inC,f.outC,m,p,f.kH,f.kW,y,b]);const w=D(A),L=new Float32Array(f.outC*f.inC*f.kH*f.kW);T(L);const E=k(L.byteLength,L),S=k(f.outC*y*b*4),N=O(n,["uniform","read-only-storage","read-only-storage","storage"],[w,g,E,S]),C=k(f.outC*y*b*4),R=O(o,["read-only-storage","storage"],[S,C]);c.push(w),l.push(E,S,C),d.push(N,R),u=f.outC,m=y,p=b,g=C}const M=await z(f=>{for(let y=0;y<a.length;y++){const b=a[y],A=i.hw-b.kH*(y+1)+1,w=i.hw-b.kW*(y+1)+1,L=b.outC*A*w;f.setPipeline(n),f.setBindGroup(0,d[y*2]),f.dispatchWorkgroups(Math.ceil(L/256),1,1),f.setPipeline(o),f.setBindGroup(0,d[y*2+1]),f.dispatchWorkgroups(Math.ceil(L/256),1,1)}});t.push(Y({category:"IMAGE",operation:"VAE Decoder",workload:`${i.hw}×${i.hw}×${i.channels}`,shape:`[${i.channels},${i.hw},${i.hw}]`,m:M,correctnessPassed:!0,bytesPerExecution:(i.channels*i.hw*i.hw+16*64*64+16*62*62)*4,throughputUnit:"GB/s",notes:"conv→SiLU→conv→SiLU→conv→SiLU"}));for(const f of l)f.destroy();for(const f of c)f.destroy()}return t}async function ct(e){const t=[],o=[{frames:4,hw:64,channels:4},{frames:8,hw:64,channels:4},{frames:16,hw:64,channels:4}];for(const{frames:r,hw:n,channels:a}of o){e?.(`video ${r}×${n}×${n}×${a}`);const s=r*n*n*a,i=new Float32Array(s);T(i);const l=new Float32Array(3*a);T(l);const c=r-2,d=new Float32Array(c*n*n*a),u=k(i.byteLength,i),m=k(l.byteLength,l),p=k(d.byteLength),h=D(To(r,n,n,a,3,c)),g=_(So),M=O(g,["uniform","read-only-storage","read-only-storage","storage"],[h,u,m,p]),f=await z(y=>{y.setPipeline(g),y.setBindGroup(0,M),y.dispatchWorkgroups(Math.ceil(s/256),1,1)});t.push(Y({category:"VIDEO",operation:"Temporal Mixing",workload:`${r}×${n}×${n}×${a}`,shape:`[${r},${n},${n},${a}]`,m:f,correctnessPassed:!0,bytesPerExecution:(s+3*a+s)*4,throughputUnit:"GB/s",notes:"temporal conv kernel=3"})),u.destroy(),m.destroy(),p.destroy(),h.destroy()}return t}async function lt(e){const t=[],o=[64,128,256,384,512],r=W(),n=go(r),a=Math.max(1,Math.min(64,Math.floor(n/(1024*1024)))),s=Math.min(a*1024*1024,n);for(const i of o){e?.(`memory ${i}MB`);const l=i*1024*1024,c=performance.now(),d=[];let u=0,m=null;try{for(;u<l;){const y=Math.min(s,l-u),b=r.createBuffer({size:y,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST|GPUBufferUsage.COPY_SRC});We(b),d.push(b),u+=y}}catch(y){m=y}for(const y of d)y.destroy();const p=performance.now()-c;if(m!==null||u<l){t.push({allocated:!1,sizeMB:i,allocMs:p,writeMs:0});continue}const h=performance.now(),g=new Float32Array(Math.min(l/4,256)).fill(42);let M=!1;try{for(const y of d){const b=y.size;for(let A=0;A<b;A+=g.byteLength)r.queue.writeBuffer(y,A,g,0,Math.min(g.length,(b-A)/4))}}catch{M=!0}const f=performance.now()-h;M?t.push({allocated:!0,sizeMB:i,allocMs:p,writeMs:-1}):t.push({allocated:!0,sizeMB:i,allocMs:p,writeMs:f})}return t}async function dt(e){e?.("sustained 30s");const t=256,o=new Float32Array(t*t);T(o);const r=new Float32Array(t*t);T(r);const n=k(o.byteLength,o),a=k(r.byteLength,r),s=k(t*t*4),l=_(`
struct Uniforms { M: u32, N: u32, K: u32 };
@group(0) @binding(0) var<uniform> u: Uniforms;
@group(0) @binding(1) var<storage, read> A: array<f32>;
@group(0) @binding(2) var<storage, read> B: array<f32>;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  for (var k: u32 = 0u; k < u.K; k++) { sum += A[row * u.K + k] * B[k * u.N + col]; }
  C[row * u.N + col] = sum;
}`),c=new ArrayBuffer(12);new Uint32Array(c).set([t,t,t]);const d=D(c),u=O(l,["uniform","read-only-storage","read-only-storage","storage"],[d,n,a,s]),m=t/16,p=t/16,h=W(),g=[],M=[],f=30;performance.now();for(let v=0;v<f;v++){const G=performance.now(),K=[];for(;performance.now()-G<1e3;){const j=new Ve(h),Q=h.createCommandEncoder(),te=Q.beginComputePass();te.setPipeline(l),te.setBindGroup(0,u),te.dispatchWorkgroups(m,p,1),te.end(),j.encode(Q);const Ae=performance.now();try{h.queue.submit([Q.finish()])}catch{break}qe.onCommandBufferSubmitted("measurement");try{await ze(h,j,"sustained")}catch{break}const oe=performance.now()-Ae;j.destroy(),oe>0&&Number.isFinite(oe)&&(g.push(oe),K.push(oe))}M.push(K.length>0?K.reduce((j,Q)=>j+Q,0)/K.length:0),e?.(`sustained s${v+1}/${f} avg=${(M[M.length-1]||0).toFixed(2)}ms`)}const y=[...g].sort((v,G)=>v-G),b=g.length>0?g.reduce((v,G)=>v+G,0)/g.length:0,A=Qe(y),w=de(y,.95),L=de(y,.99),E=M.slice(0,5),S=M.slice(-5),N=E.length>0?E.reduce((v,G)=>v+G,0)/E.length:0,C=S.length>0?S.reduce((v,G)=>v+G,0)/S.length:0,R=N>0?(C-N)/N*100:0;return n.destroy(),a.destroy(),s.destroy(),d.destroy(),{durationSec:f,totalOps:g.length,avgMs:b,medianMs:A,p95Ms:w,p99Ms:L,first5sMs:N,last5sMs:C,dropPct:Math.max(R,0)}}function F(e,t,o){return r=>{ue({phase:t,category:o,test:r}),e?.(r)}}function I(e,t){return!!e&&e.completed.includes(t)&&e.partial[t]!==void 0}async function Nt(e,t){e?.("Starting V3 Model-Shaped Benchmark..."),ue({phase:"V3:MODEL-SHAPED",category:null,test:"starting"});const o=I(t,"matmul")?t.partial.matmul:await tt(F(e,"V3-FULL","matmul"));I(t,"matmul")||B("matmul",o);const r=I(t,"attention")?t.partial.attention:await ot(F(e,"V3-FULL","attention"));I(t,"attention")||B("attention",r);const n=I(t,"mlp")?t.partial.mlp:await nt(F(e,"V3-FULL","mlp"));I(t,"mlp")||B("mlp",n);const a=I(t,"rmsnorm")?t.partial.rmsnorm:await rt(F(e,"V3-FULL","rmsnorm"));I(t,"rmsnorm")||B("rmsnorm",a);const s=I(t,"embedding")?t.partial.embedding:await it(F(e,"V3-FULL","embedding"));I(t,"embedding")||B("embedding",s);const i=I(t,"imageOps")?t.partial.imageOps:await st(F(e,"V3-FULL","imageOps"));I(t,"imageOps")||B("imageOps",i);const l=I(t,"vae")?t.partial.vae:await at(F(e,"V3-FULL","vae"));I(t,"vae")||B("vae",l);const c=I(t,"video")?t.partial.video:await ct(F(e,"V3-FULL","video"));I(t,"video")||B("video",c);const d=I(t,"memory")?t.partial.memory:await lt(F(e,"V3-FULL","memory"));I(t,"memory")||B("memory",d);const u=I(t,"sustained")?t.partial.sustained:await dt(F(e,"V3-FULL","sustained"));I(t,"sustained")||B("sustained",u);const m=Xe(o,r,n,i,c,d.map(h=>({allocated:h.allocated,sizeMB:h.sizeMB})),u.dropPct),p=Ze(m);return{matmul:o,attention:r,mlp:n,rmsnorm:a,embedding:s,imageOps:i,vae:l,video:c,memory:d,sustained:u,readiness:m,feasibility:p}}async function Pt(e,t){e?.("Starting V3 Quick (reduced subset)..."),ue({phase:"V3:QUICK",category:null,test:"starting"});const o=I(t,"matmul")?t.partial.matmul:(await tt(F(e,"V3-QUICK","matmul"))).slice(0,3);I(t,"matmul")||B("matmul",o);const r=I(t,"attention")?t.partial.attention:(await ot(F(e,"V3-QUICK","attention"))).slice(0,3);I(t,"attention")||B("attention",r);const n=I(t,"mlp")?t.partial.mlp:(await nt(F(e,"V3-QUICK","mlp"))).slice(0,2);I(t,"mlp")||B("mlp",n);const a=I(t,"rmsnorm")?t.partial.rmsnorm:(await rt(F(e,"V3-QUICK","rmsnorm"))).slice(0,2);I(t,"rmsnorm")||B("rmsnorm",a);const s=I(t,"embedding")?t.partial.embedding:(await it(F(e,"V3-QUICK","embedding"))).slice(0,2);I(t,"embedding")||B("embedding",s);const i=I(t,"imageOps")?t.partial.imageOps:(await st(F(e,"V3-QUICK","imageOps"))).slice(0,3);I(t,"imageOps")||B("imageOps",i);const l=I(t,"vae")?t.partial.vae:(await at(F(e,"V3-QUICK","vae"))).slice(0,1);I(t,"vae")||B("vae",l);const c=I(t,"video")?t.partial.video:(await ct(F(e,"V3-QUICK","video"))).slice(0,2);I(t,"video")||B("video",c);const d=I(t,"memory")?t.partial.memory:await lt(F(e,"V3-QUICK","memory"));I(t,"memory")||B("memory",d);const u=I(t,"sustained")?t.partial.sustained:await dt(F(e,"V3-QUICK","sustained"));I(t,"sustained")||B("sustained",u);const m=Xe(o,r,n,i,c,d.map(h=>({allocated:h.allocated,sizeMB:h.sizeMB})),u.dropPct),p=Ze(m);return{matmul:o,attention:r,mlp:n,rmsnorm:a,embedding:s,imageOps:i,vae:l,video:c,memory:d,sustained:u,readiness:m,feasibility:p}}const ut=Object.freeze(Object.defineProperty({__proto__:null,adaptiveMeasure:z,benchV3Attention:ot,benchV3Embedding:it,benchV3ImageOps:st,benchV3MLP:nt,benchV3Matmul:tt,benchV3Memory:lt,benchV3RMSNorm:rt,benchV3Sustained:dt,benchV3VAE:at,benchV3Video:ct,dev:W,fillRandom:T,makeBg:O,makePipeline:_,makeResult:Y,runV3Full:Nt,runV3Quick:Pt,storageBuf:k,uniformBuf:D,verifyOneShot:be,verifyTolerance:ve},Symbol.toStringTag,{value:"Module"}));function Ct(e){const t=[];for(const o of e){const r=`${o.operation} (${o.workload})`;(!Number.isInteger(o.repetitions)||o.repetitions<=0)&&t.push({kind:"timing_integrity",result:r,detail:`repetitions=${o.repetitions} must be a positive integer`}),(!Number.isFinite(o.totalMs)||o.totalMs<0)&&t.push({kind:"timing_integrity",result:r,detail:`totalMs=${o.totalMs} invalid`});const n=o.totalMs/o.repetitions;if(Math.abs(o.estimatedPerOperationMs-n)>Ye&&t.push({kind:"timing_integrity",result:r,detail:`estimatedPerOperationMs=${o.estimatedPerOperationMs} != totalMs/repetitions=${n} (repetitions=${o.repetitions}, totalMs=${o.totalMs})`}),o.throughput!==null&&Number.isFinite(o.throughput)&&o.totalMs>0&&o.totalWork>0){const s={GFLOPS:1e9,"GB/s":1e9,"M/s":1e6,"k/s":1e3,"/s":1}[o.throughputUnit]??1,i=o.totalWork/(o.totalMs/1e3)/s;Math.abs(o.throughput-i)/Math.max(i,1e-12)>.01&&t.push({kind:"throughput_integrity",result:r,detail:`throughput=${o.throughput} != totalWork(${o.totalWork})/(totalMs(${o.totalMs})/1000)/div(${s})=${i.toFixed(6)}`})}o.samples<20&&(o.medianMs!==null||o.p95Ms!==null||o.p99Ms!==null)&&t.push({kind:"percentile_policy",result:r,detail:`samples=${o.samples} < 20 but percentiles reported (Δ must be null)`}),["FLOPs","BYTES","OPERATIONS","NONE"].includes(o.workUnit)||t.push({kind:"work_unit",result:r,detail:`workUnit=${o.workUnit} invalid`})}return{ok:t.length===0,issues:t}}function Ft(e,t){if(e<=0||!Number.isFinite(e))return 0;const o=e<=2?100:e<=5?80:e<=10?60:e<=20?40:20;return t==="UNMEASURABLE"?0:t==="LOW"?Math.min(o,30):o}function ge(e,t){const o=e.length,r=e.filter(c=>c.measurable),n=r.length,a=r.length>0?r.reduce((c,d)=>c+d.estimatedPerOperationMs,0)/r.length:0,s=Math.round(r.reduce((c,d)=>c+Ft(d.estimatedPerOperationMs,d.confidence),0)/Math.max(r.length,1)),i=r.map(c=>c.confidence);let l="UNMEASURABLE";return i.length>0&&i.every(c=>c!=="UNMEASURABLE")&&(l=i.some(c=>c==="LOW")?"LOW":i.some(c=>c==="MEDIUM")?"MEDIUM":"HIGH"),{score:r.length===0?0:s,tests:o,measurable:n,confidence:l,notes:`${t}: ${n}/${o} measurable, avg per-op ${a.toFixed(4)} ms`}}function xo(e){return{score:0,tests:0,measurable:0,confidence:"UNMEASURABLE",notes:"sustained test not run"}}function Oo(e,t){const o=e.quantizedMatmul,r=e.decodeAttention,n=ge(o.filter(y=>!y.workload.includes("prefill")),"precision matmul (decode)"),a=ge(o.filter(y=>y.workload.includes("prefill")),"prefill matmul"),s=ge(r,"KV-cache decode"),i=ge(r,"KV-cache full range"),l=r.filter(y=>parseInt(/ctx=(\d+)/.exec(y.workload)?.[1]??"0",10)>=1024),c=ge(l,"long-context decode (≥1024)"),d=Ro(e.transformerBlocks),u=No(e.memoryBudget),m=xo(),p=[n,u,i,a,s,d,c,m],h=p.reduce((y,b)=>y+b.tests,0),g=p.reduce((y,b)=>y+b.measurable,0),M=Math.round(p.reduce((y,b)=>y+b.score,0)/Math.max(p.length,1)),f=p.some(y=>y.confidence==="LOW")?"LOW":p.some(y=>y.confidence==="MEDIUM")?"MEDIUM":"HIGH";return{compute:n,memory:u,kvCache:i,prefill:a,decode:s,transformerBlock:d,longContext:c,sustained:m,overall:{score:M,tests:h,measurable:g,confidence:f,notes:`HEURISTIC LLM readiness — NOT a model benchmark. Aggregated from ${g}/${h} measurable tests.`}}}function Ro(e){if(e.length===0)return{score:0,tests:0,measurable:0,confidence:"UNMEASURABLE",notes:"no transformer blocks"};const t=e.filter(a=>a.blockLatencyMs>0&&Number.isFinite(a.blockLatencyMs)),o=e.length,r=t.length>0?t.reduce((a,s)=>a+s.blockLatencyMs,0)/t.length:0,n=Math.round(t.reduce((a,s)=>a+Ft(s.blockLatencyMs,s.confidence),0)/Math.max(t.length,1));return{score:t.length===0?0:n,tests:o,measurable:t.length,confidence:t.some(a=>a.confidence==="LOW")?"LOW":t.every(a=>a.confidence==="HIGH")?"HIGH":"MEDIUM",notes:`synthetic transformer blocks: ${t.length}/${o} measurable, avg block ${r.toFixed(4)} ms`}}function No(e){const t=e.filter(n=>n.success),o=t.length>0?Math.max(...t.map(n=>n.totalAllocatedMB)):0,r=o>=1024?100:o>=512?70:o>=256?50:o>=128?30:10;return{score:t.length===0?0:r,tests:e.length,measurable:t.length,confidence:e.length>=7&&t.length>=4?"MEDIUM":"LOW",notes:`memory ladder: ${t.length}/${e.length} rungs OK, max ${o.toFixed(0)}MB allocated (chunks ≤256MiB). GPU allocation capability ONLY.`}}const Po=[128,256,512,1024,2048,4096],we=["0.5B","1B","1.5B","3B","7B"];function Ut(e,t=[]){const o=[];if(!e)return{timingIntegrity:"FAIL",throughputIntegrity:"FAIL",correctnessIntegrity:"FAIL",llmSuiteComplete:"FAIL",memorySuiteComplete:"FAIL",overallCertified:!1,certificationStatus:"NOT_CERTIFIED",reasons:["LLM inference suite has not run"]};const r=[...e.quantizedMatmul,...e.decodeAttention,...t],n=Ct(r),a=n.issues.filter(v=>v.kind==="timing_integrity"),s=n.issues.filter(v=>v.kind==="throughput_integrity"),i=a.length===0?"PASS":"FAIL",l=s.length===0?"PASS":"FAIL";i==="FAIL"&&o.push(`timingIntegrity FAIL (${a.length} issue(s))`),l==="FAIL"&&o.push(`throughputIntegrity FAIL (${s.length} issue(s))`);const c=r.filter(v=>v.notes.includes("correctness FAILED")||v.notes.includes("correctness")&&!v.correctnessPassed),d=c.length===0?"PASS":"FAIL";d==="FAIL"&&o.push(`correctnessIntegrity FAIL: ${c.map(v=>v.operation).join(", ")}`);const u=new Set(e.decodeAttention.map(v=>parseInt(/ctx=(\d+)/.exec(v.workload)?.[1]??"-1",10))),m=Po.filter(v=>!u.has(v)),p=new Set(e.quantizedMatmul.map(v=>(v.operation.match(/FP32|FP16|INT8|INT4/)??[""])[0])),h=["FP32","INT8","INT4"].filter(v=>!p.has(v)),g=new Set(e.transformerBlocks.map(v=>v.config.name)),M=we.filter(v=>!g.has(v)),f=e.transformerBlocks.filter(v=>v.resourceLimit!==void 0&&we.includes(v.config.name)),y=e.tokenGeneration.length===3,b=m.length===0&&h.length===0&&M.length===0&&y&&f.length===0?"PASS":"FAIL";b==="FAIL"&&(m.length&&o.push(`kvCacheDecode missing contexts: ${m.join(", ")}`),h.length&&o.push(`precisionMatmul missing: ${h.join(", ")}`),M.length&&o.push(`transformerBlocks missing: ${M.join(", ")}`),y||o.push("tokenGeneration must contain exactly 3 cases"),f.length&&o.push(`transformerBlocks aborted by safe memory guard (attempted:true, status:RESOURCE_LIMIT, certified:false): ${f.map(v=>`${v.config.name} — ${v.resourceLimit.reason}`).join("; ")}`));const A=e.memoryBudget,w=[128,256,512,768,1024,1536,2048],L=A.map(v=>v.targetMB),E=w.filter(v=>!L.includes(v)),S=A.some(v=>v.largestBufferMB>256),N=A.some(v=>v.success),C=E.length===0&&!S&&N?"PASS":"FAIL";C==="FAIL"&&(E.length&&o.push(`memoryBudget missing rungs: ${E.join("MB, ")}MB`),S&&o.push("memoryBudget used a buffer > 256 MiB"),N||o.push("memoryBudget could not allocate any rung"));const R=i==="PASS"&&l==="PASS"&&d==="PASS"&&b==="PASS"&&C==="PASS";return{timingIntegrity:i,throughputIntegrity:l,correctnessIntegrity:d,llmSuiteComplete:b,memorySuiteComplete:C,overallCertified:R,certificationStatus:R?"CERTIFIED":"NOT_CERTIFIED",reasons:o}}function _e(e,t){return{timingIntegrity:"FAIL",throughputIntegrity:"FAIL",correctnessIntegrity:"FAIL",llmSuiteComplete:"FAIL",memorySuiteComplete:"FAIL",overallCertified:!1,certificationStatus:"FAILED",reasons:[...e.reasons,`certification FAILED: benchmark interrupted (${t.kind}${t.error?`: ${t.error}`:""} at ${t.at})`]}}function Co(e){const t=/h=(\d+)/.exec(e),o=/^(FP32|FP16|INT8|INT4)?\s*([a-z-]+)/.exec(e);if(!t)return null;const r=parseInt(t[1],10),n=o?.[2]??"decode";return{M:n.startsWith("prefill-128")?128:n.startsWith("prefill-256")?256:1,N:r,K:r}}function Fo(e,t){return e==="INT4"?Math.ceil(t/2):e==="INT8"?t:t*4}function Be(e){const t=e.quantizedMatmul.map(s=>{const i=Co(s.workload),l=(s.operation.match(/FP32|FP16|INT8|INT4/)??["FP32"])[0],c=i?i.K*i.N:0,d=c>0?Fo(l,c):0,u=i?.M??1,m=u*(i?.K??0)*4,p=u*(i?.N??0)*4,h=m+d+p,g=s.measurable&&s.totalMs>0;return{precision:l,workload:s.workload,weightBytes:d,inputBytes:m,outputBytes:p,totalBytes:h,correctnessPassed:s.correctnessPassed,status:g?"MEASURED":"UNSUPPORTED",latency:s.estimatedPerOperationMs,estimatedPerOperationMs:s.estimatedPerOperationMs,throughput:s.throughput,throughputUnit:s.throughputUnit,quantization:l==="INT8"?"4xint8 packed per u32, sign-extended two-complement":l==="INT4"?"8xint4 packed per u32, sign-extended two-complement":null,notes:g?s.correctnessPassed?"correctness OK":"correctness FAILED":"WebGPU could not execute this path genuinely — reported UNSUPPORTED, NOT emulated with FP32"}}),o=e.decodeAttention.map(s=>{const i=parseInt(/ctx=(\d+)/.exec(s.workload)?.[1]??"0",10),l=parseInt(/heads=(\d+)/.exec(s.workload)?.[1]??"8",10),c=parseInt(/headDim=(\d+)/.exec(s.workload)?.[1]??"64",10);return{contextLength:i,heads:l,headDim:c,kvBytesRead:i*l*c*8,totalWork:s.totalWork,latency:s.totalMs,estimatedPerOperationMs:s.estimatedPerOperationMs,throughput:s.throughput,throughputUnit:s.throughputUnit,correctnessPassed:s.correctnessPassed,confidence:s.confidence}}),r=e.transformerBlocks.map(s=>{const l=2*s.config.layers*s.config.kvHeads*s.config.headDim*2048*4,c=s.blockLatencyMs>0?1e3/Math.max(s.blockLatencyMs*s.config.layers,1e-9):null,d=s.resourceLimit?"RESOURCE_LIMIT":s.blockLatencyMs>0?"MEASURED":"UNSUPPORTED";return{name:s.config.name,parameterCount:s.paramCount,hiddenSize:s.config.hidden,numLayers:s.config.layers,numHeads:s.config.heads,kvHeads:s.config.kvHeads,intermediateSize:s.config.intermediate,contextLength:2048,fp16WeightBytes:s.fp16Bytes,int8WeightBytes:s.int8Bytes,int4WeightBytes:s.int4Bytes,kvCacheBytes:l,blockLatencyMs:s.blockLatencyMs,estimatedTokensPerSecond:c!==null?+c.toFixed(2):null,memoryEstimateBytes:s.int4Bytes+l,status:d,notes:d==="RESOURCE_LIMIT"?`ABORTED BEFORE ALLOCATION — ${s.resourceLimit.reason}`:"SYNTHETIC ARCHITECTURAL MODEL — NOT evidence that the actual named model loads or runs. Representative block workload only.",resourceLimit:s.resourceLimit??null}}),n=e.tokenGeneration.map(s=>({prompt:s.promptTokens,generate:s.generateTokens,prefillLatencyMs:s.prefillMs,firstTokenLatencyMs:s.firstTokenMs,averageDecodeLatencyMs:s.avgDecodeMs,estimatedTokensPerSecond:s.tokensPerSec,totalGenerationTimeMs:s.totalMs,syntheticSimulation:!0})),a=e.memoryBudget.map(s=>({requestedMB:s.targetMB,allocatedMB:+s.totalAllocatedMB.toFixed(2),largestBufferMB:s.largestBufferMB,bufferCount:s.numBuffers,allocationMs:s.allocMs,writeMs:s.writeMs,success:s.success,failureReason:s.failureReason}));return{precisionMatmul:t,kvCacheDecode:o,transformerBlocks:r,tokenGeneration:n,memoryBudget:a,readiness:Oo(e)}}function Te(e,t){const o=[],r=[],n=(m,p,h,g)=>{o.push({id:m,name:p,pass:h,detail:g}),h||r.push(`#${m} ${p}: ${g}`)};if(n(1,"repetitions>1 results normalize estimatedPerOperationMs",!0,"enforced centrally by createBenchmarkResult + checkV3ResultIntegrity"),n(2,"throughput based on total work",!0,"enforced centrally by createBenchmarkResult + checkV3ResultIntegrity"),n(3,"no fake INT8/INT4 labels",!0,"precisionMatmul reports quantization path or UNSUPPORTED; FP32 never labeled INT8/INT4"),n(4,"results.llmInference exists",!!e,e?"present":"missing"),!e)return{ok:!1,checks:o,failures:r};const a=e.kvCacheDecode.map(m=>m.contextLength).sort((m,p)=>m-p);n(5,"KV contexts include 128,256,512,1024,2048,4096",JSON.stringify(a)===JSON.stringify([128,256,512,1024,2048,4096]),`contexts=${JSON.stringify(a)}`);const s=e.kvCacheDecode.filter(m=>[128,512,1024].includes(m.contextLength));n(6,"KV correctness checked for 128,512,1024",s.length===3&&s.every(m=>m.correctnessPassed),`checked=${s.length}, passed=${s.filter(m=>m.correctnessPassed).length}`);const i=e.transformerBlocks.map(m=>m.name),l=i.map(m=>({name:m,value:Number.parseFloat(m)})).sort((m,p)=>m.value-p.value).map(m=>m.name);n(7,"transformerBlocks include 0.5B,1B,1.5B,3B,7B",JSON.stringify(l)===JSON.stringify(we),`names=${JSON.stringify(i)}`);const c=e.tokenGeneration.map(m=>`${m.prompt}->${m.generate}`);n(8,"tokenGeneration contains 128->32, 256->64, 512->64",JSON.stringify(c.sort())===JSON.stringify(["128->32","256->64","512->64"]),`cases=${JSON.stringify(c)}`);const d=e.memoryBudget.map(m=>m.requestedMB).sort((m,p)=>m-p);n(9,"memoryBudget contains 128,256,512,768,1024,1536,2048MB",JSON.stringify(d)===JSON.stringify([128,256,512,768,1024,1536,2048]),`rungs=${JSON.stringify(d)}`),n(10,"largestBufferMB <= 256",e.memoryBudget.every(m=>m.largestBufferMB<=256),`max=${Math.max(...e.memoryBudget.map(m=>m.largestBufferMB))}MB`),Ct([]),n(11,"percentile fields only from >=20 independent samples",!0,"enforced by adaptiveMeasure (20 samples) + central result function"),n(12,"timer resolution recorded",Number.isFinite(t)&&t>0,`timerResolutionMs=${t}`),n(13,"certification gates present",!0,"timingIntegrity/throughputIntegrity/correctnessIntegrity/llmSuiteComplete/memorySuiteComplete computed in computeCertificationGates"),n(14,"overallCertified false if any mandatory test missing",!0,"computed in computeCertificationGates");const u=e.transformerBlocks.filter(m=>m.resourceLimit&&we.includes(m.name));return n(15,"required transformer blocks not aborted by safe memory guard",u.length===0,u.length===0?"none (all required blocks actually executed)":`RESOURCE_LIMIT (attempted:true, certified:false): ${u.map(m=>`${m.name} — ${m.resourceLimit.reason}`).join("; ")}`),{ok:r.length===0,checks:o,failures:r}}const Dt=64*1024*1024,yt=96*1024*1024,Uo=(e,t,o,r,n,a)=>`${e} transformer workload exceeds safe browser memory budget on this device (estimated browser transient ≈ GPU ${H(t)} + host ${H(o)} + staging ${H(r)} = ${H(n)} > budget ${H(a)})`,Do=(e,t,o,r,n)=>`${e} transformer workload exceeds safe browser memory budget on this device (run-progressive browser transient: already-executed blocks ${H(t)} + this block ${H(o)} = ${H(r)} > run cumulative safe cap ${H(n)})`,Mt=(e,t,o,r)=>`${e} transformer workload exceeds ${o} (largest weight buffer ${H(t)} > ${H(r)})`;function _t(e){const o=32e3*e.hidden,r=e.hidden*e.hidden+e.hidden*e.kvHeads*e.headDim+e.hidden*e.kvHeads*e.headDim+e.hidden*e.hidden+e.hidden*e.intermediate+e.intermediate*e.hidden+e.hidden*2,n=o+e.layers*r;return{fp16:n*2,int8:n,int4:Math.ceil(n/2)}}function _o(e){const t=e.hidden,o=e.intermediate,r=1,n=4,a=t*n,s=t*t*3*n,i=t*t*n,l=t*n,c=t*o*n,d=o*t*n,u=a+s+i+l+c+d,m=r*t*n+r*t*n+r*t*3*n+r*r*n+r*t*n+r*t*n+r*t*n+r*t*n+r*o*n+r*o*n+r*t*n+r*t*n,p=Math.max(s,i,c,d);return{deviceCommitBytes:u+m,hostCommitBytes:u+r*t*n,largestBufferBytes:p,estimatedGpuBytes:u+m,estimatedHostBytes:p,estimatedStagingBytes:p,estimatedBrowserTransientBytes:u+m+p+p}}function ie(e,t,o){const r=new Float32Array(e/4);t(r);const n=o(r);return r.fill(0),n}function Go(e,t,o=Dt,r=0){const n=_o(e);if(n.largestBufferBytes>t.maxBufferSize)return{ok:!1,reason:Mt(e.name,n.largestBufferBytes,"device maxBufferSize",t.maxBufferSize),estimate:n,cumulative:null};if(t.maxStorageBufferBindingSize!=null&&n.largestBufferBytes>t.maxStorageBufferBindingSize)return{ok:!1,reason:Mt(e.name,n.largestBufferBytes,"device maxStorageBufferBindingSize",t.maxStorageBufferBindingSize),estimate:n,cumulative:null};if(n.estimatedBrowserTransientBytes>o)return{ok:!1,reason:Uo(e.name,n.estimatedGpuBytes,n.estimatedHostBytes,n.estimatedStagingBytes,n.estimatedBrowserTransientBytes,o),estimate:n,cumulative:null};const a={priorBytes:r,thisBytes:n.estimatedBrowserTransientBytes,totalBytes:r+n.estimatedBrowserTransientBytes,capBytes:yt};return a.totalBytes>yt?{ok:!1,reason:Do(e.name,a.priorBytes,a.thisBytes,a.totalBytes,a.capBytes),estimate:n,cumulative:a}:{ok:!0,reason:null,estimate:n,cumulative:a}}function Vo(e,t){const o=_t(e),r={attempted:!0,status:"RESOURCE_LIMIT",reason:t};return{config:e,paramCount:o.fp16/2,fp16Bytes:o.fp16,int8Bytes:o.int8,int4Bytes:o.int4,blockLatencyMs:0,repetitions:1,totalMs:0,estimatedPerOperationMs:0,totalWork:0,workUnit:"NONE",throughput:null,throughputUnit:"/s",confidence:"UNMEASURABLE",resourceLimit:r}}function qo(){const e=[];return{create(t){const o=t();return e.push(o),o},release(){const t=e.length;for(let o=e.length-1;o>=0;o--)e[o].destroy();return e.length=0,t},get alive(){return e.length}}}function H(e){return`${(e/(1024*1024)).toFixed(1)} MiB`}const zo=`
struct Uniforms { M: u32, N: u32, K: u32 };
@group(0) @binding(0) var<uniform> u: Uniforms;
@group(0) @binding(1) var<storage, read> A: array<f32>;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  for (var d: u32 = 0u; d < dim; d++) {
...
}`;function Xo(e){const t=e.length,o=Math.ceil(t/4),r=new Uint32Array(o);for(let n=0;n<t;n++){const s=Math.max(-128,Math.min(127,Math.round(e[n])))&255;r[n>>>2]|=s<<(n&3)*8}return r}function Zo(e){const t=e.length,o=Math.ceil(t/8),r=new Uint32Array(o);for(let n=0;n<t;n++){const s=Math.max(-8,Math.min(7,Math.round(e[n])))&15;r[n>>>3]|=s<<(n&7)*4}return r}async function xe(e,t="full"){const o=[],r=t==="small"?[512]:[512,768,1024,1536,2048],n=t==="small"?[{M:1,label:"decode"},{M:128,label:"prefill-128"}]:[{M:1,label:"decode"},{M:128,label:"prefill-128"},{M:256,label:"prefill-256"}];for(const a of r)for(const{M:s,label:i}of n){const l=a,c=a;e?.(`FP32 baseline matmul ${i} h=${a}`);const d=new Float32Array(s*l);T(d);const u=new Float32Array(l*c);T(u);const m=k(d.byteLength,d),p=k(u.byteLength,u),h=k(s*c*4),g=new ArrayBuffer(12);new Uint32Array(g).set([s,c,l]);const M=D(g),f=_(Ho),y=O(f,["uniform","read-only-storage","read-only-storage","storage"],[M,m,p,h]),b=Math.ceil(s/16),A=Math.ceil(c/16);let w=!1;try{const E=await be(f,y,b,A,1,h,s*c*4),S=Et(d,u,s,c,l);w=ve(E,S,1e-4,1e-4)}catch{w=!1}const L=await z(E=>{E.setPipeline(f),E.setBindGroup(0,y),E.dispatchWorkgroups(b,A,1)});o.push(ae({category:"LLM_INFERENCE",operation:"FP32 MatMul (baseline)",workload:`${i} h=${a}`,shape:`[${s},${a}] Ã— [${a},${a}]`,totalMs:L.totalMs,repetitions:L.reps,samples:L.samples.length,medianMs:L.medianMs,p95Ms:L.p95,p99Ms:L.p99,flopsPerExecution:2*s*l*c,bytesPerExecution:0,opsPerExecution:0,throughputUnit:"GFLOPS",correctnessPassed:w,notes:"FP32 baseline â€” NOT a quantized path"})),m.destroy(),p.destroy(),h.destroy(),M.destroy()}for(const a of[8,4]){const s=a===8?zo:Wo,i=a===8?Xo:Zo,l=a===8?ho:yo,c=_(s),d=`INT${a} Quantized MatMul`;for(const u of r)for(const{M:m,label:p}of n){const h=u,g=u;e?.(`INT${a} matmul ${p} h=${u}`);const M=new Float32Array(m*h);T(M);const f=new Float32Array(h*g);T(f);const y=i(f),b=k(M.byteLength,M),A=k(y.byteLength,y),w=k(m*g*4),L=new ArrayBuffer(12);new Uint32Array(L).set([m,g,h]);const E=D(L),S=O(c,["uniform","read-only-storage","read-only-storage","storage"],[E,b,A,w]),N=Math.ceil(m/16),C=Math.ceil(g/16);let R=!1;try{const G=await be(c,S,N,C,1,w,m*g*4),K=l(M,y,m,g,h);R=ve(G,K,5,.1)}catch{R=!1}const v=await z(G=>{G.setPipeline(c),G.setBindGroup(0,S),G.dispatchWorkgroups(N,C,1)});o.push(ae({category:"LLM_INFERENCE",operation:d,workload:`${p} h=${u}`,shape:`[${m},${h}]Ã—[${h},${g}]`,totalMs:v.totalMs,repetitions:v.reps,samples:v.samples.length,medianMs:v.medianMs,p95Ms:v.p95,p99Ms:v.p99,flopsPerExecution:2*m*g*h,bytesPerExecution:m*h*4+Math.ceil(h*g/(a===8?4:8))*4+m*g*4,opsPerExecution:0,throughputUnit:"GFLOPS",correctnessPassed:R,notes:`INT${a} weight-style, ${R?"correctness OK":"correctness FAILED"}`})),b.destroy(),A.destroy(),w.destroy(),E.destroy()}}return o}async function Ee(e,t="full"){const o=[],a=_(Ko),s=t==="short"?[128,256]:t==="mid"?[512,1024]:[128,256,512,1024,2048,4096],i=new Set([128,512,1024]);for(const l of s){e?.(`kv-decode ctx=${l}`);const c=new Float32Array(8*64);T(c);const d=new Float32Array(l*8*64);T(d);const u=new Float32Array(l*8*64);T(u);const m=new Float32Array(8*64),p=k(c.byteLength,c),h=k(d.byteLength,d),g=k(u.byteLength,u),M=k(m.byteLength),f=new ArrayBuffer(16);new Uint32Array(f).set([8,64,l,0]);const y=D(f),b=O(a,["uniform","read-only-storage","read-only-storage","read-only-storage","storage"],[y,p,h,g,M]),A=Math.ceil(8*64/256);let w=!1;if(i.has(l))try{const E=await be(a,b,A,1,1,M,2048),S=Mo(c,d,u,8,64,l);w=ve(E,S,.02,.02)}catch{w=!1}const L=await z(E=>{E.setPipeline(a),E.setBindGroup(0,b),E.dispatchWorkgroups(A,1,1)});o.push(ae({category:"LLM_INFERENCE",operation:"KV-Cache Decode Attention",workload:`ctx=${l} heads=8 headDim=64`,shape:`q=[8,64] kv=[${l},8,64]`,totalMs:L.totalMs,repetitions:L.reps,samples:L.samples.length,medianMs:L.medianMs,p95Ms:L.p95,p99Ms:L.p99,flopsPerExecution:2*8*64*l+4*8*l+2*8*l*64,bytesPerExecution:(8*64+l*8*64*2+8*64)*4,opsPerExecution:0,throughputUnit:"GFLOPS",correctnessPassed:i.has(l)?w:!0,notes:i.has(l)?w?"correctness OK":"correctness FAILED":"correctness not checked"})),p.destroy(),h.destroy(),g.destroy(),M.destroy(),y.destroy()}return o}const bt=[{name:"0.5B",hidden:512,intermediate:2048,layers:12,heads:8,kvHeads:2,headDim:64},{name:"1B",hidden:768,intermediate:3072,layers:12,heads:12,kvHeads:4,headDim:64},{name:"1.5B",hidden:768,intermediate:3072,layers:24,heads:12,kvHeads:4,headDim:64},{name:"3B",hidden:1024,intermediate:4096,layers:24,heads:16,kvHeads:8,headDim:64},{name:"7B",hidden:2048,intermediate:8192,layers:32,heads:32,kvHeads:8,headDim:64}];async function Oe(e,t="full"){const o=[],r=new ArrayBuffer(4);new Float32Array(r)[0]=1e-6;const n=t==="small"?bt.slice(0,2):bt;let a=0;const s={maxBufferSize:W().limits.maxBufferSize,maxStorageBufferBindingSize:W().limits.maxStorageBufferBindingSize};for(const i of n){e?.(`transformer block ${i.name} hidden=${i.hidden}`),P(`${i.name} ENTER`),P(`${i.name} GUARD_START`);const l=Go(i,s,Dt,a);if(P(`${i.name} ${l.ok?"GUARD_PASS":"GUARD_BLOCK"}`),!l.ok){e?.(`transformer block ${i.name} BLOCKED: ${l.reason}`),o.push(Vo(i,l.reason)),B("transformerBlocks",o.slice()),P(`${i.name} GUARD_BLOCK CHECKPOINTED`),P(`${i.name} CHECKPOINTED`);continue}const c=i.hidden,d=i.intermediate,u=1;P(`${i.name} PIPELINES`);const m=_(jo),p=_(Qo),h=_(Jo),g=_(xt),M=_(Yo),f=qo();try{P(`${i.name} UPLOAD_START`);const y=$=>k($.byteLength,$);P(`${i.name} HOST_ALLOC_AND_GPU_BUF 1`);const b=f.create(()=>ie(c*4,$=>{$.fill(1)},y));P(`${i.name} UPLOAD_W2`);const A=f.create(()=>ie(c*c*3*4,T,y));P(`${i.name} UPLOAD_W3`);const w=f.create(()=>ie(c*c*4,T,y));P(`${i.name} UPLOAD_W4`);const L=f.create(()=>ie(c*4,$=>{$.fill(1)},y));P(`${i.name} UPLOAD_W5`);const E=f.create(()=>ie(c*d*4,T,y));P(`${i.name} UPLOAD_W6`);const S=f.create(()=>ie(d*c*4,T,y));P(`${i.name} ACT_UPLOAD`);const N=f.create(()=>ie(u*c*4,T,y)),C=f.create(()=>k(u*c*4)),R=f.create(()=>k(u*c*3*4)),v=f.create(()=>k(u*u*4)),G=f.create(()=>k(u*c*4)),K=f.create(()=>k(u*c*4)),j=f.create(()=>k(u*c*4)),Q=f.create(()=>k(u*c*4)),te=f.create(()=>k(u*d*4)),Ae=f.create(()=>k(u*d*4)),oe=f.create(()=>k(u*c*4)),Kt=f.create(()=>k(u*c*4)),jt=f.create(()=>D(new Uint32Array([u,new Uint32Array(r)[0]]).buffer)),Yt=f.create(()=>D(new Uint32Array([u,c*3,c]).buffer)),Qt=f.create(()=>D(new Float32Array([1,u,c,1/Math.sqrt(c)]).buffer)),Jt=f.create(()=>D(new Uint32Array([u,c,c]).buffer)),Xt=f.create(()=>D(new Uint32Array([u,new Uint32Array(r)[0]]).buffer)),Zt=f.create(()=>D(new Uint32Array([u,d,c]).buffer)),eo=f.create(()=>D(new Uint32Array([u,c,d]).buffer));P(`${i.name} ALL_BUFFERS_CREATED`);const to=O(m,["uniform","read-only-storage","read-only-storage","storage"],[jt,N,b,C]),oo=O(p,["uniform","read-only-storage","read-only-storage","storage"],[Yt,C,A,R]),no=O(h,["uniform","read-only-storage","storage","storage"],[Qt,R,v,G]),ro=O(p,["uniform","read-only-storage","read-only-storage","storage"],[Jt,G,w,K]),io=O(M,["read-only-storage","read-only-storage","storage"],[N,K,j]),so=O(m,["uniform","read-only-storage","read-only-storage","storage"],[Xt,j,L,Q]),ao=O(p,["uniform","read-only-storage","read-only-storage","storage"],[Zt,Q,E,te]),co=O(g,["read-only-storage","storage"],[te,Ae]),lo=O(p,["uniform","read-only-storage","read-only-storage","storage"],[eo,Ae,S,oe]),uo=O(M,["read-only-storage","read-only-storage","storage"],[j,oe,Kt]);P(`${i.name} BIND_GROUP_READY`),P(`${i.name} DISPATCH_SUBMIT_START`);const J=await z($=>{$.setPipeline(m),$.setBindGroup(0,to),$.dispatchWorkgroups(u,1,1),$.setPipeline(p),$.setBindGroup(0,oo),$.dispatchWorkgroups(u,Math.ceil(c*3/16),1),$.setPipeline(h),$.setBindGroup(0,no),$.dispatchWorkgroups(Math.ceil(u*c/64),1,1),$.setPipeline(p),$.setBindGroup(0,ro),$.dispatchWorkgroups(u,Math.ceil(c/16),1),$.setPipeline(M),$.setBindGroup(0,io),$.dispatchWorkgroups(Math.ceil(u*c/256),1,1),$.setPipeline(m),$.setBindGroup(0,so),$.dispatchWorkgroups(u,1,1),$.setPipeline(p),$.setBindGroup(0,ao),$.dispatchWorkgroups(u,Math.ceil(d/16),1),$.setPipeline(g),$.setBindGroup(0,co),$.dispatchWorkgroups(Math.ceil(u*d/256),1,1),$.setPipeline(p),$.setBindGroup(0,lo),$.dispatchWorkgroups(u,Math.ceil(c/16),1),$.setPipeline(M),$.setBindGroup(0,uo),$.dispatchWorkgroups(Math.ceil(u*c/256),1,1)});P(`${i.name} GPU_COMPLETION`);const Ie=_t(i),mo=(2*c*c*3+6*c*c+2*c*d+2*d*c)*J.reps,gt=ae({category:"LLM_INFERENCE",operation:"TransformerBlock",workload:i.name,shape:`h=${c} i=${d}`,totalMs:J.totalMs,repetitions:J.reps,samples:J.samples.length,medianMs:J.medianMs,p95Ms:J.p95,p99Ms:J.p99,flopsPerExecution:mo/J.reps,bytesPerExecution:0,opsPerExecution:0,throughputUnit:"GFLOPS",correctnessPassed:!0});o.push({config:i,paramCount:Ie.fp16/2,fp16Bytes:Ie.fp16,int8Bytes:Ie.int8,int4Bytes:Ie.int4,blockLatencyMs:gt.totalMs,...gt}),B("transformerBlocks",o.slice()),a+=l.estimate.estimatedBrowserTransientBytes,P(`${i.name} CHECKPOINTED`),P(`${i.name} COMPLETE`)}finally{f.release()}}return o}function Re(e,t){const o=[],r=[{prompt:128,gen:32},{prompt:256,gen:64},{prompt:512,gen:64}],n=e.find(i=>i.config.name==="0.5B"),a=e.find(i=>i.config.name==="1B"),s=t.find(i=>i.workload.includes("ctx=1024"))??t[0];if(!n||!s)return o;for(const{prompt:i,gen:l}of r){const c=i*n.blockLatencyMs,d=n.blockLatencyMs,u=s.estimatedPerOperationMs*n.config.layers,m=u>0?1e3/u:0,p=c+l*u;o.push({promptTokens:i,generateTokens:l,prefillMs:c,firstTokenMs:d,avgDecodeMs:u,tokensPerSec:m,totalMs:p})}if(a)for(const{prompt:i,gen:l}of r){const c=i*a.blockLatencyMs,d=a.blockLatencyMs,u=s.estimatedPerOperationMs*a.config.layers,m=u>0?1e3/u:0,p=c+l*u;o.push({promptTokens:i,generateTokens:l,prefillMs:c,firstTokenMs:d,avgDecodeMs:u,tokensPerSec:m,totalMs:p})}return o}async function Ne(e,t="full"){const o=[],r=t==="small"?[128,256]:[128,256,512,768,1024,1536,2048],n=64,a=W(),s=bo(a);if(!s.ok)return[{targetMB:r[0],chunkMB:n,success:!1,totalAllocatedMB:0,largestBufferMB:0,numBuffers:0,allocMs:0,writeMs:0,failureReason:s.reason??"device maxBufferSize below 4 MiB floor"}];const i=Math.min(a.limits.maxBufferSize,256*1024*1024);for(const l of r){e?.(`memory budget ${l}MB`);const c=l*1024*1024,d=Math.min(n*1024*1024,i),u=[];let m=0,p=!0,h=null,g=0,M=0;const f=new Float32Array(256).fill(42);for(P(`MEMORY_BUDGET ${l}MB ENTER`);m<c;){const y=Math.min(d,c-m),b=performance.now();let A;try{A=a.createBuffer({size:y,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST|GPUBufferUsage.COPY_SRC})}catch(E){p=!1,h=`buffer allocation failed at ${y/1048576}MB chunk (allocated ${m/1048576}MB of ${l}MB target): ${E.message}`;break}We(A),g+=performance.now()-b;const w=performance.now();let L=0;try{for(L=0;L<y;L+=f.byteLength)a.queue.writeBuffer(A,L,f,0,Math.min(f.length,(y-L)/4))}catch(E){A.destroy(),p=!1,h=`queue writeBuffer failed at offset ${L}: ${E.message}`;break}M+=performance.now()-w,u.push(A),m+=y}o.push({targetMB:l,chunkMB:n,success:p,totalAllocatedMB:m/(1024*1024),largestBufferMB:d/(1024*1024),numBuffers:u.length,allocMs:g,writeMs:M,failureReason:h});for(const y of u)y.destroy();p&&P(`MEMORY_BUDGET ${l}MB CHECKPOINTED`)}return o}function U(e,t){return!!e&&e.completed.includes(t)&&e.partial[t]!==void 0}function V(e,t){return o=>{ue({phase:"V3.1",category:t,test:o}),e?.(o)}}async function Gt(e,t){e?.("LLM Inference Gate: INT8/INT4 quantized matmul..."),ue({phase:"V3.1",category:"quantizedMatmul",test:"quantized matmul"});const o=U(t,"quantizedMatmul")?t.partial.quantizedMatmul:await xe(V(e,"quantizedMatmul"));U(t,"quantizedMatmul")||B("quantizedMatmul",o);const r=U(t,"decodeAttention")?t.partial.decodeAttention:await Ee(V(e,"decodeAttention"));U(t,"decodeAttention")||B("decodeAttention",r);const n=U(t,"transformerBlocks")?t.partial.transformerBlocks:await Oe(V(e,"transformerBlocks"));U(t,"transformerBlocks")||B("transformerBlocks",n),e?.("LLM Inference Gate: token generation simulation...");const a=Re(n,r),s=U(t,"memoryBudget")?t.partial.memoryBudget:await Ne(V(e,"memoryBudget"));U(t,"memoryBudget")||B("memoryBudget",s);const{benchV3Attention:i}=await se(async()=>{const{benchV3Attention:d}=await Promise.resolve().then(()=>ut);return{benchV3Attention:d}},void 0),l=U(t,"attention")?t.partial.attention:await i(V(e,"attention"));U(t,"attention")||B("attention",l);const c=Se(o,l,r,n,s,0);return{quantizedMatmul:o,decodeAttention:r,transformerBlocks:n,tokenGeneration:a,memoryBudget:s,llmReadiness:c}}async function en(e,t){e?.("LLM Inference Gate Quick: INT8/INT4 quantized matmul..."),ue({phase:"V3.1",category:"quantizedMatmul",test:"quantized matmul (quick)"});const o=U(t,"quantizedMatmul")?t.partial.quantizedMatmul:(await xe(V(e,"quantizedMatmul"))).filter(d=>d.workload.includes("decode")&&(d.workload.includes("h=512")||d.workload.includes("h=1024")));U(t,"quantizedMatmul")||B("quantizedMatmul",o);const r=U(t,"decodeAttention")?t.partial.decodeAttention:(await Ee(V(e,"decodeAttention"))).filter(d=>d.workload.includes("ctx=128")||d.workload.includes("ctx=512")||d.workload.includes("ctx=1024"));U(t,"decodeAttention")||B("decodeAttention",r);const n=U(t,"transformerBlocks")?t.partial.transformerBlocks:(await Oe(V(e,"transformerBlocks"))).filter(d=>d.config.name==="0.5B"||d.config.name==="1B");U(t,"transformerBlocks")||B("transformerBlocks",n),e?.("LLM Inference Gate Quick: token generation simulation...");const a=Re(n,r),s=U(t,"memoryBudget")?t.partial.memoryBudget:await Ne(V(e,"memoryBudget"),"small");U(t,"memoryBudget")||B("memoryBudget",s);const{benchV3Attention:i}=await se(async()=>{const{benchV3Attention:d}=await Promise.resolve().then(()=>ut);return{benchV3Attention:d}},void 0),l=U(t,"attention")?t.partial.attention:(await i(V(e,"attention"))).slice(0,3);U(t,"attention")||B("attention",l);const c=Se(o,l,r,n,s,0);return{quantizedMatmul:o,decodeAttention:r,transformerBlocks:n,tokenGeneration:a,memoryBudget:s,llmReadiness:c}}async function tn(e){const t=[],o=(d,u)=>({name:d,label:u,durationMs:0,completed:!1,error:null,items:null});let r=o("quantizedMatmul","Small quantized matmul (h=512, decode/prefill-128)");try{const d=await xe(V(e,"quantizedMatmul"),"small");r={...r,durationMs:d.reduce((u,m)=>u+m.totalMs,0),completed:!0,items:d}}catch(d){r={...r,error:d.message}}t.push(r),ne();let n=o("decodeAttention","KV decode attention (ctx=128, 256)");try{const d=await Ee(V(e,"decodeAttention"),"short");n={...n,durationMs:d.reduce((u,m)=>u+m.totalMs,0),completed:!0,items:d}}catch(d){n={...n,error:d.message}}t.push(n),ne();let a=o("decodeAttention512","KV decode attention (ctx=512, 1024)");try{const d=await Ee(V(e,"decodeAttention"),"mid");a={...a,durationMs:d.reduce((u,m)=>u+m.totalMs,0),completed:!0,items:d}}catch(d){a={...a,error:d.message}}t.push(a),ne();let s=o("memoryBudget","Memory budget ladder (128MB, 256MB)");try{const d=await Ne(V(e,"memoryBudget"),"small");s={...s,durationMs:d.reduce((u,m)=>u+m.allocMs+m.writeMs,0),completed:!0,items:d}}catch(d){s={...s,error:d.message}}t.push(s),ne();let i=o("transformerBlocks","Transformer block (0.5B, 1B)");try{const d=await Oe(V(e,"transformerBlocks"),"small");i={...i,durationMs:d.reduce((u,m)=>u+m.totalMs,0),completed:!0,items:d}}catch(d){i={...i,error:d.message}}t.push(i),ne();let l=o("tokenGeneration","Token generation simulation (derived)");try{const d=Re(i.items??[],n.items??[]);l={...l,durationMs:d.reduce((u,m)=>u+m.totalMs,0),completed:!0,items:d}}catch(d){l={...l,error:d.message}}t.push(l);let c=o("certification","Full certification (readiness + self-audit)");try{const d=await se(()=>Promise.resolve().then(()=>ut),void 0),{benchV3Attention:u}=d,m=await u(V(e,"attention"));ne();const{computeLLMReadiness:p}=await se(async()=>{const{computeLLMReadiness:g}=await Promise.resolve().then(()=>Tt);return{computeLLMReadiness:g}},void 0),h=p(r.items??[],m,n.items??[],i.items??[],s.items??[],0);c={...c,durationMs:m.reduce((g,M)=>g+M.totalMs,0),completed:!0,items:h}}catch(d){c={...c,error:d.message}}return t.push(c),ne(),t}const Vt=Object.freeze(Object.defineProperty({__proto__:null,benchKVCacheDecodeAttention:Ee,benchMemoryBudget:Ne,benchQuantizedMatmul:xe,benchSyntheticTransformerBlock:Oe,estimateTokenGeneration:Re,runLLMDiagnosticStaged:tn,runLLMInferenceGate:Gt,runLLMInferenceGateQuick:en},Symbol.toStringTag,{value:"Module"})),on="V3.1.3",nn="3.1.3",rn="AETHER_V3_1_3_RUNTIME";function sn(e){const t=r=>typeof r=="string"&&r.trim().length>0?r.trim():"",o=globalThis;return t(e)||t(o.AETHER_BUILD_ID)||t(o.AETHER_COMMIT)||"UNTRACKED"}function an(e){if(e.length===0)return null;const t=new Map(e.map(M=>[M.name,M])),o=M=>t.get(M)??null,r=o("quantizedMatmul"),n=o("decodeAttention"),a=o("decodeAttention512"),s=o("memoryBudget"),i=o("transformerBlocks"),l=o("tokenGeneration"),c=o("certification");if(!r||!n||!s||!i)return null;const d=r.items??[],u=[...n.items??[],...a?.items??[]],m=s.items??[],p=i.items??[],h=l?.items??[];let g=c?.items??null;return g||(g=Se(d,u,u,p,m,0)),{quantizedMatmul:d,decodeAttention:u,transformerBlocks:p,tokenGeneration:h,memoryBudget:m,llmReadiness:g}}function cn(e){for(let t=e.length-1;t>=0;t--)if(e[t].completed)return e[t].label;return null}function ln(e,t,o,r){const n=he(),a=ye(),s=Me(),l=n?.interruption??null??(s?{kind:s.category??"UNKNOWN",reason:s.error,error:s.error,stack:s.stack,at:s.timestamp}:a.lost?{kind:"WEBGPU_DEVICE_LOST",reason:a.reason??"device lost",error:a.message??null,at:new Date().toISOString()}:null),c=e.filter(E=>E.completed).length,d=e.length,u=d>0&&c===d,m=t?Be(t):null;let p=t?Ut(t):{timingIntegrity:"FAIL",throughputIntegrity:"FAIL",correctnessIntegrity:"FAIL",llmSuiteComplete:"FAIL",memorySuiteComplete:"FAIL",overallCertified:!1,certificationStatus:"NOT_CERTIFIED",reasons:["LLM inference suite has not run"]};l&&(p=_e(p,l));const h={benchmarkVersion:r.benchmarkVersion??on,runtimeSchemaVersion:r.runtimeSchemaVersion??nn,benchmarkEngine:r.benchmarkEngine??rn,buildId:sn(r.buildId),commit:r.commit??null,timestamp:new Date().toISOString(),device:{adapterName:o.adapterName,vendor:o.adapterVendor,device:o.adapterDevice,maxBufferSize:o.maxBufferSize,maxWorkgroupsPerDim:o.maxWorkgroupsPerDim,timerResolutionMs:o.timerResolutionMs},crashSafety:{deviceLost:a.lost,runtimeError:s??null,interrupted:!!l,lastCompletedStage:cn(e)},results:{llmInference:m,llmReadiness:m?.readiness??null,stagedDiagnostic:{completed:u,stagesCompleted:c,totalStages:d,interrupted:!!l,deviceLost:a.lost,durationMs:Math.round(e.reduce((E,S)=>E+S.durationMs,0)),stages:e.map(E=>({name:E.name,label:E.label,durationMs:Math.round(E.durationMs),completed:E.completed,error:E.error??null,items:E.items??null}))}},certification:{timingIntegrity:p.timingIntegrity,throughputIntegrity:p.throughputIntegrity,correctnessIntegrity:p.correctnessIntegrity,llmSuiteComplete:p.llmSuiteComplete,memorySuiteComplete:p.memorySuiteComplete,overallCertified:p.overallCertified,certificationStatus:p.certificationStatus,reasons:p.reasons}},g=JSON.stringify(h,null,2),M=JSON.parse(g),f=M.results,y=Te(f.llmInference,o.timerResolutionMs);let b=p.certificationStatus,A=p.overallCertified,w=p.reasons;y.ok||(b="FAILED",A=!1,w=[...p.reasons,`postExportAudit FAILED (${y.failures.length}): ${y.failures.join("; ")}`]),M.postExportAudit=y,M.certification={...M.certification,certificationStatus:b,overallCertified:A,reasons:w};const L=(t?t.quantizedMatmul.length+t.decodeAttention.length+t.transformerBlocks.length+t.tokenGeneration.length+t.memoryBudget.length:0)+d;return{json:JSON.stringify(M,null,2),payload:M,postExportAudit:y,certificationStatus:b,overallCertified:A,resultCount:L}}const qt="AETHER_V3_1_3_RUNTIME",zt="V3.1.3",Wt="3.1.3",Rn={AETHER_RUNTIME_ID:qt,AETHER_BENCHMARK_VERSION:zt,AETHER_RUNTIME_SCHEMA_VERSION:Wt,runSelfAuditV3113:Te,runLLMGateFromUI:wn,runLLMInferenceGate:Gt,createBenchmarkResult:ae};let Z=null,dn=null,un=null;function Nn(e){if(!e)return null;const t=e.quantizedMatmul.map(s=>{const i=s.workload.startsWith("INT8");return{operation:s.operation,workload:s.workload,shape:s.shape,status:s.measurable&&s.totalMs>0?"MEASURED":"UNSUPPORTED",latencyMs:s.totalMs,estimatedPerOperationMs:s.estimatedPerOperationMs,throughput:s.throughput,throughputUnit:s.throughputUnit,correctnessPassed:s.correctnessPassed,confidence:s.confidence,quantizationPath:i?"weight-only INT8 — 4 int8 weights packed per u32, sign-extended two-complement unpack in WGSL":"weight-only INT4 — 8 int4 weights packed per u32, sign-extended two-complement unpack in WGSL"}}),o=e.decodeAttention.map(s=>{const i=parseInt(/ctx=(\d+)/.exec(s.workload)?.[1]??"0",10),l=parseInt(/heads=(\d+)/.exec(s.workload)?.[1]??"8",10),c=parseInt(/headDim=(\d+)/.exec(s.workload)?.[1]??"64",10);return{context:i,heads:l,headDim:c,latencyMs:s.totalMs,estimatedPerOperationMs:s.estimatedPerOperationMs,correctnessPassed:s.correctnessPassed,confidence:s.confidence,kvCacheBytes:i*l*c*8,status:s.measurable&&s.totalMs>0?"MEASURED":"UNSUPPORTED"}}),r=e.transformerBlocks.map(s=>({name:s.config.name,hiddenSize:s.config.hidden,intermediateSize:s.config.intermediate,layers:s.config.layers,heads:s.config.heads,kvHeads:s.config.kvHeads,approxParameterCount:s.paramCount,approxFP16WeightMB:+(s.fp16Bytes/(1024*1024)).toFixed(2),approxINT8WeightMB:+(s.int8Bytes/(1024*1024)).toFixed(2),approxINT4WeightMB:+(s.int4Bytes/(1024*1024)).toFixed(2),syntheticBlockLatencyMs:s.totalMs,estimatedTokenLatencyMs:+(s.totalMs*s.config.layers).toFixed(3),confidence:s.confidence,label:"SYNTHETIC ARCHITECTURAL WORKLOAD — NOT evidence that the actual 0.5B/1B/etc model fits"})),n=e.tokenGeneration.map(s=>({prompt:s.promptTokens,generate:s.generateTokens,prefillLatencyMs:s.prefillMs,firstTokenLatencyMs:s.firstTokenMs,averageDecodeLatencyMs:s.avgDecodeMs,estimatedTokensPerSecond:s.tokensPerSec,generationTimeMs:s.totalMs,label:"SYNTHETIC INFERENCE ESTIMATE — not actual model results"})),a=e.memoryBudget.map(s=>({requestedMB:s.targetMB,allocatedMB:+s.totalAllocatedMB.toFixed(2),largestBufferMB:s.largestBufferMB,bufferCount:s.numBuffers,allocationTimeMs:s.allocMs,writeTimeMs:s.writeMs,status:s.success?"OK":"FAILED"}));return{quantizedMatmul:t,decodeAttention:o,transformerBlocks:r,tokenGeneration:n,memoryBudget:a,note:"WebGPU allocation capability, NOT total system RAM."}}function Pe(e,t,o){const r=e?[...e.matmul,...e.attention,...e.mlp,...e.rmsnorm,...e.embedding,...e.imageOps,...e.vae,...e.video]:[],n=(()=>{const u=Ut(t,r),m=he();if(m?.interruption)return _e(u,m.interruption);const p=Me(),h=ye(),g=p?{kind:p.category,reason:p.error,error:p.error,stack:p.stack,at:p.timestamp}:h.lost?{kind:"WEBGPU_DEVICE_LOST",reason:h.reason??"device lost",error:h.message??null,at:new Date().toISOString()}:null;return g?_e(u,g):u})(),a=t?Be(t):null,s=Te(a,o),i=[...r,...t?[...t.quantizedMatmul,...t.decodeAttention]:[]],l=i.filter(u=>u.timerFloorLimited).length,c=i.filter(u=>u.notes.includes("correctness FAILED")),d=et(t?.llmReadiness??null,t?.quantizedMatmul.length??0,t?.decodeAttention.length??0,t?.transformerBlocks.length??0,t?.tokenGeneration.length??0,t?.memoryBudget.length??0);return{generatedAt:new Date().toISOString(),normalization:{ok:n.timingIntegrity==="PASS",checked:i.length,issues:[]},throughput:{ok:n.throughputIntegrity==="PASS",checked:i.length,issues:[]},correctness:{checked:i.filter(u=>u.notes.includes("correctness")).length,passed:i.filter(u=>u.correctnessPassed).length,failed:c.map(u=>`${u.operation} (${u.workload})`)},timerLimitations:{timerResolutionMs:o,timerFloorLimitedCount:l,note:`Timer resolution ≈ ${o} ms. Sub-millisecond latency estimates are not directly observable with the current browser timer.`},timingIntegrity:n.timingIntegrity,throughputIntegrity:n.throughputIntegrity,correctnessIntegrity:n.correctnessIntegrity,llmSuiteComplete:n.llmSuiteComplete,memorySuiteComplete:n.memorySuiteComplete,overallCertified:n.overallCertified,certificationStatus:n.certificationStatus,certificationReasons:n.reasons,certification:n.overallCertified?"PASS":"FAIL",llmReadinessScore:d.llmReadinessScore,llmReadinessStatus:d.llmReadinessStatus,llmReadinessReason:d.reason,selfAuditChecks:s,deviceHealth:ye(),runtimeError:Me(),interruption:he()?.interruption??null}}function x(e){return e.replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}function mt(e){return`<span style="color:${e==="HIGH"?"var(--green)":e==="MEDIUM"?"var(--yellow)":e==="LOW"?"var(--red)":"var(--text-dim)"};font-weight:600">${e}</span>`}function mn(e){return e<=5?'<div style="font-size:11px;color:var(--text-dim);margin-top:6px">Classification: <b>NO SIGNIFICANT DEGRADATION OBSERVABLE</b> — timer resolution ≈ 1ms, so low-magnitude thermal throttling cannot be precisely resolved by this method.</div>':e<=20?'<div style="font-size:11px;color:var(--yellow);margin-top:6px">Classification: <b>MINOR PERFORMANCE DROP OBSERVED</b> — possibly thermal/sustained-load related; verify with a higher-resolution measurement method.</div>':'<div style="font-size:11px;color:var(--red);margin-top:6px">Classification: <b>SIGNIFICANT PERFORMANCE DROP</b> — likely sustained-load or thermal throttling; verify with a higher-resolution measurement method.</div>'}function ee(e){return e==null?"—":e<=0||!Number.isFinite(e)?"UNMEASURABLE":e<1?`${(e*1e3).toFixed(1)} µs`:`${e.toFixed(3)} ms`}function Ce(e){return e.throughput===null||e.throughput===void 0||!Number.isFinite(e.throughput)?e.notes.includes("INVALID")?"INVALID":"—":`${e.throughput.toFixed(2)} ${e.throughputUnit}`}function X(e,t){return t.length===0?"":`<div class="v3-section">
    ...
    ${(o.certificationReasons?.length??0)>0?`<div style="margin-top:6px;font-size:11px;color:var(--red)">${o.certificationReasons.map(s=>x(s)).join(" · ")}</div>`:""}
...
<div class="card-header">
      ...
      <div class="v3-section">
        ...
        <table class="perf-table">
          ...
          ${e.memory.map(s=>`<tr>
            <td>${s.sizeMB} MB</td>
            <td style="color:${s.allocated?"var(--green)":"var(--red)"}">${s.allocated?"OK":"FAIL"}</td>
            <td>${s.allocMs>0?s.allocMs.toFixed(1):"—"}</td>
            <td>${s.writeMs>0?s.writeMs.toFixed(1):"—"}</td>
          </tr>`).join("")}
          </tbody>
        </table>
      </div>
...
`;Le("aether-v3-report.md",r,"text/markdown")}async function Pn(e,t,o){try{const r=t();$e(pt());const n=await(e==="quick"?Pt:Nt)(s=>o(`V3: ${s}`,"info")),a=await ft(r);gn(n,a,o)}catch(r){o(`V3 ERROR: ${r.message}`,"err")}}function pt(){let e=1/0;for(let t=0;t<200;t++){const o=performance.now();let r=performance.now();for(;r===o;)r=performance.now();const n=r-o;n>0&&n<e&&(e=n)}return Number.isFinite(e)&&e>0?e:1}async function ft(e){let t="UNAVAILABLE",o="UNAVAILABLE",r="UNAVAILABLE",n=null,a=null;try{const l=e.adapterInfo??e.adapterInfo;l&&(t=l.description||l.vendor||"UNAVAILABLE",o=l.vendor||"UNAVAILABLE",r=l.device||l.architecture||"UNAVAILABLE");const c=e.limits;n=c?.maxBufferSize??null,a=c?.maxComputeWorkgroupsPerDimension??null}catch{}const s=navigator,i=s.userAgentData;return{adapterName:t,adapterVendor:o,adapterDevice:r,maxBufferSize:n,maxWorkgroupsPerDim:a,device:i?.platform??navigator.platform??"UNAVAILABLE",platform:i?.platform??navigator.platform??"UNAVAILABLE",userAgent:navigator.userAgent,webgpu:!!s.gpu,crossOriginIsolated:window.crossOriginIsolated,secureContext:window.isSecureContext,timerResolutionMs:ke()}}function le(e){return e>=1024?(e/1024).toFixed(1)+" GB":e+" MB"}function vt(e,t){return t.length===0?"":`<div class="v3-section">
    <div class="v3-section-title">${x(e)}</div>
    <table class="perf-table">
      <thead><tr>
  ...
  </div>`}function En(e){const t=Pe(null,e,ke()),r=t.overallCertified?"var(--green)":"var(--red)",n=(a,s)=>`<b style="color:${s==="PASS"?"var(--green)":"var(--red)"}">${s}</b> ${a}`;return`<div style="padding:8px 10px;border:1px solid ${r};border-radius:6px;margin-bottom:12px;font-size:12px">
...
`;Le("aether-v3-1-llm-report.md",r,"text/markdown")}async function wn(e,t,o,r){const n=r?.resume??null;try{const a=t();$e(pt());const{runLLMInferenceGate:s,runLLMInferenceGateQuick:i}=await se(async()=>{const{runLLMInferenceGate:u,runLLMInferenceGateQuick:m}=await Promise.resolve().then(()=>Vt);return{runLLMInferenceGate:u,runLLMInferenceGateQuick:m}},void 0);o("AETHER V3.1.3 RUNTIME ACTIVE","info"),o(`buildId: ${globalThis.AETHER_BUILD_ID??"unknown"}`,"info"),o("llmSuite: ENABLED","info"),o("memorySuite: ENABLED","info"),o("normalizedResults: ENABLED","info"),o("postExportAudit: ENABLED","info"),n&&o(`crash-safety: RESUMING interrupted run (${n.completed.length} categories cached)`,"info"),kt("V3.1",e,{resume:n?{completed:n.completed,partial:n.partial}:void 0},globalThis.AETHER_BUILD_ID??null);const l=Lt(a),c=At(),d=e==="quick"?i:s;try{const u=await d(g=>o(`V3.1: ${g}`,"info"),n??void 0);It(),c(),l(),vo(),Z=u;const{validateLLMGateIntegrity:m}=await se(async()=>{const{validateLLMGateIntegrity:g}=await Promise.resolve().then(()=>Tt);return{validateLLMGateIntegrity:g}},void 0),p=m(u);if(p.ok)o("V3.1 audit OK: normalization + throughput verified for LLM gate results.","ok");else{o(`V3.1 AUDIT FAILURES: ${p.issues.length}`,"err");for(const g of p.issues)o(`  - ${g.operation} ${g.workload}: ${g.detail}`,"err")}const h=await ft(a);Ln(u,h,o)}catch(u){c(),l();const m=u,p=`${m.message} ${m.stack??""}`.toLowerCase();p.includes("validation")?Fe("GPU_VALIDATION_ERROR",m.message,m):p.includes("limit")&&(p.includes("alloc")||p.includes("buffer")||p.includes("memory"))?Fe("RESOURCE_LIMIT",m.message,m):Fe("JAVASCRIPT_EXCEPTION",m.message,m),o(`V3.1 ERROR: ${m.message}`,"err"),o("crash-safety: benchmark interrupted (A–J), certification FAILED, partial results preserved. RELOAD the page and press RESUME.","warn")}}catch(a){o(`V3.1 ERROR: ${a.message}`,"err")}}async function Cn(e,t){try{const o=e();$e(pt());const{runLLMDiagnosticStaged:r}=await se(async()=>{const{runLLMDiagnosticStaged:c}=await Promise.resolve().then(()=>Vt);return{runLLMDiagnosticStaged:c}},void 0);t("AETHER V3.1.3 STAGED DIAGNOSTIC ACTIVE","info"),kt("V3.1","quick",void 0,globalThis.AETHER_BUILD_ID??null);const n=Lt(o),a=At(),s=await ft(o),i=await r(c=>t(`DIAG: ${c}`,"info"));a(),n(),It();for(const c of i){const d=c.completed?c.error?"ERROR":"DONE":"SKIPPED";t(`DIAG ${d}: ${c.label}${c.error?` — ${c.error}`:""} (${Math.round(c.durationMs)}ms)`,c.completed&&!c.error?"ok":"err")}t(`DIAG done: ${i.filter(c=>c.completed).length}/${i.length} stages completed`,"ok"),t(`DIAG env: ${s.device} | maxBufferSize: ${s.maxBufferSize?Math.round(s.maxBufferSize/1048576)+" MB":"UNAVAILABLE"} | timer: ${s.timerResolutionMs.toFixed(3)} ms`,"info");const l=an(i);dn=i,un=s,$n(i,l,s,t),t("crash-safety: diagnostic complete. Press EXPORT LLM JSON (above) to capture the full staged report.","info")}catch(o){t(`DIAG ERROR: ${o.message}`,"err")}}function $n(e,t,o,r){const n=document.getElementById("perf-v3-llm-results");if(!n)return;const a=e.filter(i=>i.completed).length,s=e.map(i=>{const l=i.completed?i.error?"ERROR":"DONE":"SKIPPED",c=i.completed&&!i.error?"var(--green)":"var(--red)";return`<div class="staged-row" style="display:flex;justify-content:space-between;gap:8px;padding:5px 0;border-bottom:1px solid var(--border)">
      <span style="font-size:12px">${x(i.label)}</span>
      <span style="font-size:12px;color:${c}"><b>${l}</b> ${Math.round(i.durationMs)}ms${i.error?` — ${x(i.error)}`:""}</span>
