...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  C[row * u.N + col] = sum;
}`),y=new ArrayBuffer(12);new Uint32Array(y).set([a,s,i]);const b=_(y),A=O(f,["uniform","read-only-storage","read-only-storage","storage"],[b,p,h,g]),w=Math.ceil(a/16),L=Math.ceil(s/16),E=await z(N=>{N.setPipeline(f),N.setBindGroup(0,A),N.dispatchWorkgroups(w,L,1)});let S=!1;try{const N=await be(f,A,w,L,1,g,d),P=Et(u,m,a,s,i);S=ve(N,P)}catch{S=!1}t.push(Y({category:"TRANSFORMER",operation:"MatMul",workload:`${r}×${n} × ${n}×${n}`,shape:`[${r},${n}]×[${n},${n}]`,m:E,correctnessPassed:S,flopsPerExecution:2*a*s*i,bytesPerExecution:(a*i+i*s+a*s)*4,throughputUnit:"GFLOPS",notes:S?"":"correctness FAILED"})),p.destroy(),h.destroy(),g.destroy(),b.destroy()}return t}async function ot(e){const t=[],o=[{hidden:512,heads:8,headDim:64,seqs:[64,128,256,512]},{hidden:768,heads:12,headDim:64,seqs:[64,128,256]}];for(const{hidden:r,heads:n,headDim:a,seqs:s}of o)for(const i of s){e?.(`attention hidden=${r} seq=${i}`);const l=1,c=a,d=i*i*4,u=i*c*4,m=new Float32Array(l*i*c*3);T(m);const p=k(m.byteLength,m),h=k(d),g=k(u),f=D(`
...
}`),y=1/Math.sqrt(c),b=new ArrayBuffer(16);new Uint32Array(b).set([l,i,c]),new Float32Array(b)[3]=y;const A=_(b),w=O(f,["uniform","read-only-storage","storage","storage"],[A,p,h,g]),L=Math.max(1,Math.ceil(l*i/64)),E=await z(S=>{S.setPipeline(f),S.setBindGroup(0,w),S.dispatchWorkgroups(L,1,1)});t.push(Y({category:"ATTENTION",operation:"Fused Attention",workload:`hidden=${r} seq=${i}`,shape:`[1,${i},${c}]`,m:E,correctnessPassed:!0,flopsPerExecution:4*l*i*i*c,bytesPerExecution:(l*i*c*3+i*i+i*c)*4,throughputUnit:"GFLOPS",notes:"QK^T+softmax+PV fused"})),p.destroy(),h.destroy(),g.destroy(),A.destroy()}return t}async function nt(e){const t=[],o=[{hidden:512,intermediate:2048,seqs:[128,256,512]},{hidden:768,intermediate:3072,seqs:[128,256]},{hidden:1024,intermediate:4096,seqs:[128]}],r=D(xt);for(const{hidden:n,intermediate:a,seqs:s}of o)for(const i of s){e?.(`mlp hidden=${n} intermediate=${a} seq=${i}`);const l=new Float32Array(i*n);T(l);const c=new Float32Array(n*a);T(c);const d=new Float32Array(a*n);T(d);const u=k(l.byteLength,l),m=k(c.byteLength,c),p=k(i*a*4),h=k(i*a*4),g=k(i*n*4),f=D(`
struct Uniforms { M: u32, N: u32, K: u32 };
@group(0) @binding(0) var<uniform> u: Uniforms;
@group(0) @binding(1) var<storage, read> A: array<f32>;
@group(0) @binding(2) var<storage, read> B: array<f32>;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  for (var k: u32 = 0u; k < u.K; k++) { sum += A[row * u.K + k] * B[k * u.N + col]; }
  C[row * u.N + col] = sum;
}`),y=new ArrayBuffer(12);new Uint32Array(y).set([i,a,n]);const b=_(y),A=O(f,["uniform","read-only-storage","read-only-storage","storage"],[b,u,m,p]),w=O(r,["read-only-storage","storage"],[p,h]),L=i*a,E=new ArrayBuffer(12);new Uint32Array(E).set([i,n,a]);const S=_(E),N=O(f,["uniform","read-only-storage","read-only-storage","storage"],[S,h,g,u]),P=await z(R=>{R.setPipeline(f),R.setBindGroup(0,A),R.dispatchWorkgroups(Math.ceil(i/16),Math.ceil(a/16),1),R.setPipeline(r),R.setBindGroup(0,w),R.dispatchWorkgroups(Math.ceil(L/256),1,1),R.setPipeline(f),R.setBindGroup(0,N),R.dispatchWorkgroups(Math.ceil(i/16),Math.ceil(n/16),1)});t.push(Y({category:"MLP",operation:"Transformer MLP",workload:`h=${n} int=${a} seq=${i}`,shape:`[${i},${n}]`,m:P,correctnessPassed:!0,flopsPerExecution:2*i*n*a+i*a+2*i*a*n,bytesPerExecution:(i*n+n*a+i*a+a*n+i*n)*4,throughputUnit:"GFLOPS",notes:"W1→GELU→W2"})),u.destroy(),m.destroy(),p.destroy(),h.destroy(),g.destroy(),b.destroy(),S.destroy()}return t}async function rt(e){const t=[],r=D(`
